# Graph Report - Notted  (2026-08-19)

## Corpus Check
- 1263 files · ~1,221,066 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 10866 nodes · 26851 edges · 510 communities (410 shown, 100 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 216 edges (avg confidence: 0.75)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b6b61cab`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- shared-validators/src/index.ts
- schema/index.ts
- document.schema.ts
- button.tsx
- AuthService
- TaskListView.tsx
- health.module.ts
- request-json.ts
- suggestion-modules.test.ts
- requestJson
- job-registry.ts
- app.config.ts
- NotesService
- task.schema.ts
- export-generation.service.ts
- CustomImage.ts
- page-geometry.ts
- openapi.builder.ts
- editor-harness.tsx
- docx.ts
- .run
- getRequestId
- presence-store.ts
- attachments.controller.test.ts
- email-renderer.service.ts
- auth.module.ts
- webhooks/requests.ts
- markdown.ts
- redirects.ts
- config.module.ts
- file-signature.ts
- realtime.gateway.ts
- auth/requests.ts
- image-transfer.ts
- workspace.schema.ts
- keyboard-shortcuts.ts
- workspaces.service.ts
- server-notes.ts
- rate-limit.guard.test.ts
- TasksService
- toolbar-commands.ts
- whereWorkspace
- image-processing.service.ts
- autosave-machine.ts
- comment.schema.ts
- export-formats.spec.ts
- createTenantContext
- api-key-auth.service.ts
- server-workspaces.ts
- search.schema.ts
- GlobalSearchDialog.tsx
- TiptapEditor.tsx
- image-upload-placeholder.ts
- 1. Workspace-scoping strategy
- storage-maintenance.service.ts
- notes.service.ts
- note-export-source.service.ts
- dev-tooling.mjs
- AttachmentDialogs.tsx
- shared-types/src/index.ts
- .transaction
- image-uploads.ts
- tags.service.ts
- ProjectsService
- dependencies
- tasks.service.ts
- tenant/index.ts
- PageContainer.tsx
- scripts
- note.ts
- AuthController
- compilerOptions
- public-environment.ts
- DashboardShell.tsx
- devDependencies
- tasks/index.ts
- image-processing.service.test.ts
- queue-worker-processor.service.ts
- image-signature.ts
- ExportService
- hybrid-search.service.ts
- dependencies
- Part 62 — Export job lifecycle
- RequireAuthorization
- RedisService
- NoteExportSourceService
- editor-tables.test.tsx
- devDependencies
- meilisearch.service.ts
- pdf-preview.ts
- devDependencies
- shared-validators/package.json
- auth.schema.ts
- NoteCollaborationProvider
- DatabaseService
- completed-parts/README.md
- project.schema.ts
- NoteIndexRepository
- auth-email-producer.service.ts
- embedding-reindex.ts
- note-index.repository.ts
- shared-types/package.json
- What You Must Do When Invoked
- Part 42 — Editor image insertion
- What You Must Do When Invoked
- compilerOptions
- NoteProjectionRepository
- CommentsService
- QueueInfrastructureService
- TagsController
- IsoTimestamp
- task.ts
- authorization-entry.service.ts
- note-images.spec.ts
- task-list.spec.ts
- StorageMaintenanceService
- export.ts
- DatabaseTransaction
- compilerOptions
- task-views.spec.ts
- README.md
- storage/index.ts
- common.schema.ts
- better-auth.setup.ts
- Part 48 — Add board, calendar, and progress views
- scripts
- scripts
- WebhooksController
- queue-infrastructure.service.ts
- webhooks/index.ts
- Part 43 — Add image manipulation UI
- Part 45 — Add storage quotas and cleanup
- attachments.service.ts
- shell/requests.ts
- TasksController
- TaskStatusesController
- extensions/index.ts
- components.json
- note-attachments.spec.ts
- project-board.spec.ts
- Part 41 — Implement image ingestion and processing
- ProjectsController
- Part 40 — Build secure object-storage services
- Part 44 — Implement generic attachment flows
- Part 47 — Implement standalone task CRUD and list view
- Part 49 — Add project board and timeline views
- Core Features Specification
- compilerOptions
- compilerOptions
- ImageToolbar.tsx
- VersionHistory.tsx
- filename.ts
- tags-templates.spec.ts
- Coverage remediation — making `pnpm test:ci` pass
- Part 21 — Integrate Better Auth on the backend
- Part 46 — Implement tags and templates
- attachment.ts
- ObjectStorageService
- search.service.ts
- NoteSharesController
- Part 50 — Establish BullMQ queues and workers
- compilerOptions
- Part 58 — Integrate Yjs collaborative editing
- task-board.test.tsx
- Part 04 — Scaffold the Next.js Web Application
- Part 11 — Implement Configuration and Dependency Clients
- Part 22 — Build authentication screens and route protection
- Part 23 — Add OAuth, two-factor authentication, passkeys, and session controls
- Part NN — Part Title
- webhook.schema.ts
- Notted
- webhooks.service.test.ts
- .info
- hybrid-ranking.service.ts
- Part 01 — Record architecture decisions and resolve specification gaps
- Part 02 — Initialize the pnpm/Turborepo monorepo
- Part 03 — Establish formatting, linting, and commit quality gates
- Part 05 — Scaffold the NestJS API Application
- Part 06 — Create Shared Types and Validators
- Part 07 — Add continuous integration
- Part 08 — Define Environment Contracts
- Part 09 — Build the Development Compose Stack
- Part 10 — Add Developer Commands and Onboarding Documentation
- Part 12 — Configure Drizzle ORM and Migration Tooling
- Part 13 — Implement Identity and Authentication Tables
- Part 14 — Implement Workspace and Membership Tables
- Part 15 — Implement Projects, Notes, Hierarchy, and Ordering
- Part 18 — Operations and Integration Tables
- Part 20 — Create realistic seed data
- Part 26 — Implement workspace lifecycle APIs
- Part 27 — Build workspace screens
- Part 28 — Implement membership and invitation flows
- Part 29 — Implement project CRUD APIs
- Part 33 — Establish the TipTap document contract
- Part 34 — Build the basic editor and toolbar
- Part 35 — Add tables, checklists, markdown shortcuts, and block behavior
- Part 36 — Build slash commands and mentions
- Part 37 — Implement the A4/Letter page container
- Part 38 — Add page breaks, focus mode, and print styling
- Part 39 — Implement reliable save behavior
- Third-Party Notices
- Notted — Corporate Notes Platform
- WorkspacesService
- NoteDetailView.tsx
- FakeXhr
- All-in-Docker development stack
- Part 16 — Implement Tags, Attachments, Comments, and Note Versions
- Part 17 — Implement Standalone Task Data
- Part 19 — Database-level tenant protection and retention policies
- Part 30 — Build project list and detail screens
- Part 31 — Implement core note APIs
- Part 32 — Build note browsing and hierarchy UI
- Part 51 — Meilisearch indexing pipeline
- Local development
- Database Schema (Drizzle ORM)
- globalDependencies
- comment.ts
- RealtimeGateway
- NoteEditorSurface.tsx
- WorkspacesController
- notes/view-preference.ts
- Notted Coding Conventions
- Part 24 — Implement centralized authorization
- Decision
- package.json
- inputs
- tasks
- print-export.spec.ts
- ApiResponse
- shared-types/tsconfig.json
- shared-validators/tsconfig.json
- test:ci
- realtime.integration.test.ts
- svg-safety.ts
- webhooks.e2e.test.ts
- search.ts
- Disposable end-to-end stack, and the production defect it uncovered
- Part 25 — Build the dashboard shell
- Phase 2 — Local Infrastructure and Configuration
- Phase 3 — Database Design and Tenant Safety
- .prettierrc.json
- build
- Notted Agent Instructions
- Part 59 — Presence and collaboration UI
- Part 60 — Inline comments, mentions, and notifications
- main.ts
- note-collaboration.projection.ts
- graphify reference: extra exports and benchmark
- Deliver a Notted Plan Part
- BrowserPoolService
- graphify reference: extra exports and benchmark
- export-pdf.integration.test.ts
- NoteSharesService
- latestActionLink
- presence-client.ts
- attachment-storage-key.ts
- Environment contracts
- ADR 0001: pnpm and Turborepo monorepo boundaries
- ADR 0004: Yjs collaborative document authority over Socket.io transport
- ApiKeys.tsx
- overrides
- note-yjs-document.ts
- Phase 1 — Product Decisions and Repository Foundation
- Phase 5 — Core Workspace, Project, and Note Management
- Phase 6 — Rich Editor and Paper Experience
- Build and Diagnose Notted Playwright Integration Tests
- nest-cli.json
- note-version-checkpoint.policy.ts
- app/layout.tsx
- agents/backend-platform-engineer.md
- agents/frontend-editor-engineer.md
- agents/lead-part-engineer.md
- agents/quality-reviewer.md
- Build and Diagnose Notted Playwright Integration Tests
- Database migration policy
- ADR 0002: Separate first-party tRPC and public REST transports
- ADR 0005: Store private binaries in MinIO with PostgreSQL metadata
- ADR 0010: Persist Better Auth sessions and encrypted authentication-email intent
- ADR 0011: Explicit durable project restriction state
- Phase 7 — Images and Attachments
- Phase 10 — Versioning, Comments, and Real-Time Collaboration
- Phase 11 — Export, Email, API Keys, and Webhooks
- Notted Development Plan
- tsconfig.json
- collaboration.spec.ts
- Part 63 — PDF and HTML export
- graphify reference: query, path, explain
- Build Notted Frontend and Editor Features
- graphify reference: query, path, explain
- src/format-bytes.ts
- Phase 4 — Authentication, Authorization, and Application Shell
- Phase 9 — Search and Indexing
- inputs
- Build Notted Frontend and Editor Features
- Deliver a Notted Plan Part
- api/package.json
- webhook-url-guard.ts
- Part 52 — Authorized full-text search
- pdf-export.service.test.ts
- shell.schema.ts
- Build Notted Backend and Data Features
- Assure Notted Quality and Operations
- Part 53 — Embeddings and semantic search
- Part 54 — Hybrid ranking
- [projectId]/page.tsx
- advanced-auth.spec.ts
- Phase 16 — Release Preparation and Incremental Delivery
- Build Notted Backend and Data Features
- Assure Notted Quality and Operations
- database.migration.test.ts
- lib/shared-contracts.ts
- graphify reference: add a URL and watch a folder
- graphify reference: commit hook and native CLAUDE.md integration
- graphify reference: incremental update and cluster-only
- opencode.json
- .opencode/opencode.json
- graphify reference: add a URL and watch a folder
- graphify reference: commit hook and native CLAUDE.md integration
- graphify reference: incremental update and cluster-only
- common/shared-contracts.ts
- (auth)/layout.tsx
- globals.test.ts
- app/not-found.tsx
- graphify reference: GitHub clone and cross-repo merge
- graphify reference: transcribe video and audio
- Part 61 — Email subsystem
- graphify.js
- graphify reference: GitHub clone and cross-repo merge
- graphify reference: transcribe video and audio
- ensure-docker-mounts.mjs
- Part 56 — Version history, diff, and restore
- StorageBucket
- storage-maintenance.integration.test.ts
- Part 64 — Markdown, TXT, DOCX and ZIP export, and the export UI
- multipart-upload.parser.ts
- PresenceBar.tsx
- note-collaboration-provider.ts
- realtime.gateway.test.ts
- Part 55 — Version snapshots and retention
- Part 57 — Build authenticated Socket.io infrastructure
- project.ts
- pnpm
- StructuredLogger
- class-variance-authority
- common.ts
- next.config.js
- next-env.d.ts
- realtime-socket.adapter.ts
- @better-auth/passkey
- export.service.ts
- MemoryObjectStore
- lowlight
- @notted/shared-types
- Phase 8 — Tasks, Tags, Templates, and Advanced Organization
- authorization-http.guard.ts
- folder-controls.test.tsx
- NoteBoard.tsx
- ApiKeysController
- @tanstack/react-query
- @tiptap/core
- user-color.ts
- storage-report.ts
- TagsService
- note-collaboration-provider.test.ts
- bull-board.service.ts
- @tiptap/extension-table-cell
- @tiptap/extension-table-row
- @better-auth/passkey
- api-keys.e2e.test.ts
- ai-provider-rate-limiter.service.ts
- class-transformer
- @tiptap/react
- class-validator
- dev-origins.test.ts
- highlight-js.d.ts
- .claude/CLAUDE.md
- .claude/skills/graphify/references/extraction-spec.md
- init-minio.sh
- decisions/README.md
- Resources
- architecture.md
- backend.md
- database.md
- frontend.md
- observability.md
- operations.md
- security.md
- eslint.config.mjs
- api-keys.service.test.ts
- NoteTimeline.tsx
- .opencode/skills/graphify/references/extraction-spec.md
- _test-eslint-fix.mjs
- playwright.config.ts
- cron-parser
- @notted/shared-validators
- webhook-sender.ts
- express
- fflate
- @nestjs/websockets
- pino
- upload-request.ts
- Public REST API
- @socket.io/redis-adapter
- Part 65 — Implement public REST API and API key management
- yjs
- Part 66 — Implement webhooks and delivery logs
- @vitest/coverage-v8
- @radix-ui/react-dialog
- SearchResultRepository
- better-auth
- FakeObjectStore
- @notted/shared-types
- highlight.js
- lucide-react
- drizzle-orm
- react
- react-dom
- socket.io-client
- .runMaintenance
- @tiptap/extension-highlight
- @tiptap/extension-mention
- @tiptap/extension-table-header
- task-calendar.test.tsx
- y-prosemirror
- SelectChain
- workspaceId
- helmet
- tasks/view-preference.ts
- minio
- web/package.json
- socket.io
- ShellService
- embedding-provider.ts
- src/auth.ts
- @dnd-kit/sortable
- @notted/shared-validators
- next
- qrcode.react
- decodeAwarenessClientIds
- sonner
- @tiptap/extension-code-block-lowlight
- @tiptap/extension-link
- @tiptap/extension-superscript
- @tiptap/extension-task-list
- @tiptap/extension-text-align
- @tiptap/pm
- note-board.test.tsx
- presence-bar.test.tsx
- NotificationService
- MemoryObjectStore
- StubObjectStore
- HealthController
- queue-worker-processor.service.test.ts
- OutboxDispatcherService
- tag.ts
- Webhooks
- MemoryObjectStore
- page-layout.spec.ts
- tag-manager.test.tsx
- .deliver
- ShareModal.tsx
- export-object-key.ts
- api-keys/index.ts
- Phase 13 — Enterprise Controls and Customization
- QueueAdminRemediationService
- api-key-secret.ts
- @better-auth/drizzle-adapter
- api-keys.service.ts
- StorageMaintenanceScheduler
- presence-color.ts
- SelectChain
- .attach
- compression
- heic-convert
- meilisearch
- pg
- rxjs
- yjs
- zod

## God Nodes (most connected - your core abstractions)
1. `DatabaseTransaction` - 224 edges
2. `whereWorkspace()` - 183 edges
3. `DatabaseService` - 162 edges
4. `StructuredLogger` - 135 edges
5. `TenantContextService` - 135 edges
6. `RequireAuthorization()` - 106 edges
7. `NotesService` - 103 edges
8. `AuthorizationEntryService` - 91 edges
9. `AuthService` - 87 edges
10. `createTenantContext()` - 83 edges

## Surprising Connections (you probably didn't know these)
- `bearerSecret()` --references--> `API_KEY_SECRET_PATTERN`  [EXTRACTED]
  apps/api/src/api-keys/api-key-auth.service.ts → packages/shared-validators/src/api-key.schema.ts
- `setupBetterAuth()` --references--> `ApiError`  [EXTRACTED]
  apps/api/src/auth/better-auth.setup.ts → packages/shared-types/src/api.ts
- `job()` --indirect_call--> `workspaceId()`  [INFERRED]
  apps/web/src/components/notes/export-note-dialog.test.tsx → apps/api/src/tasks/task-statuses.controller.ts
- `note()` --indirect_call--> `workspaceId()`  [INFERRED]
  apps/web/src/components/notes/note-board.test.tsx → apps/api/src/tasks/task-statuses.controller.ts
- `item()` --indirect_call--> `workspaceId()`  [INFERRED]
  apps/web/src/components/notes/note-list.test.tsx → apps/api/src/tasks/task-statuses.controller.ts

## Import Cycles
- 3-file cycle: `apps/api/src/database/schema/notes.ts -> apps/api/src/database/schema/tasks.ts -> apps/api/src/database/schema/tags.ts -> apps/api/src/database/schema/notes.ts`
- 4-file cycle: `apps/api/src/app.module.ts -> apps/api/src/openapi/openapi.module.ts -> apps/api/src/openapi/openapi.controller.ts -> apps/api/src/openapi/openapi.builder.ts -> apps/api/src/app.module.ts`

## Communities (510 total, 100 thin omitted)

### Community 0 - "shared-validators/src/index.ts"
Cohesion: 0.03
Nodes (119): ATTACHMENT_FILE_EXTENSIONS, ATTACHMENT_FILE_MIME_TYPES, ATTACHMENT_IMAGE_MIME_TYPES, ATTACHMENT_INLINE_MIME_TYPES, ATTACHMENT_TEXT_EXTENSIONS, ATTACHMENT_TEXT_MIME_TYPE, ATTACHMENT_UPLOAD_ACCEPT, attachmentBlurPlaceholderSchema (+111 more)

### Community 1 - "schema/index.ts"
Cohesion: 0.02
Nodes (135): aiProviderConfig, aiProviderConfigRelations, aiProviderEnum, aiUsage, aiUsageRelations, ADR-0007, apiIdempotencyRecords, apiKeysRelations (+127 more)

### Community 2 - "document.schema.ts"
Cohesion: 0.04
Nodes (126): assertMigrationInputBounds(), attachmentPlainText(), ATTRLESS_MARK_TYPES, BLOCK_NODE_TYPES, cellAttributesHtml(), cellSpanOrNull(), cellWidthOrNull(), CODE_LANGUAGE_ALIASES (+118 more)

### Community 3 - "button.tsx"
Cohesion: 0.06
Nodes (52): Error(), ErrorProps, ReauthenticationDialogProps, AttachmentDeleteDialog(), ColorPickerDialog(), ColorPickerDialogProps, EditorColorOption, ImageAltTextDialog() (+44 more)

### Community 4 - "AuthService"
Cohesion: 0.04
Nodes (74): request(), principal(), request(), AuthGuard, Injectable, AUTH_PRINCIPAL, AuthenticatedRequest, getAuthPrincipal() (+66 more)

### Community 5 - "TaskListView.tsx"
Cohesion: 0.05
Nodes (60): CreateTaskForm(), loadFailure(), MyTasksWidget(), alpha, beta, gamma, mocks, page (+52 more)

### Community 6 - "health.module.ts"
Cohesion: 0.05
Nodes (45): MINIO_CONFIG, MinioConfig, REDIS_CONFIG, RedisConfig, SMTP_CONFIG, SmtpConfig, DatabaseReadinessIndicator, Injectable (+37 more)

### Community 7 - "request-json.ts"
Cohesion: 0.07
Nodes (36): job(), mocks, open(), view(), ExportNoteDialog(), failureCopy(), requestFailureCopy(), advance() (+28 more)

### Community 8 - "suggestion-modules.test.ts"
Cohesion: 0.04
Nodes (75): activeOptionIndex(), COMMAND_EXPECTATIONS, CommandContext, EMPTY_DOCUMENT, menuOptions(), openSlashMenu(), optionLabelled(), slashMenu() (+67 more)

### Community 9 - "requestJson"
Cohesion: 0.10
Nodes (63): CreateNoteDialog(), depths(), FolderControls(), NoteBrowser(), VIEW_MODES, runAction(), RenameNoteControl(), createApiKey() (+55 more)

### Community 10 - "job-registry.ts"
Cohesion: 0.03
Nodes (87): jobOutbox, JobOutboxPayload, InsertedRow, mentionInput, ADR-0006, Injectable, WORKSPACE_EMAIL_IDEMPOTENCY_PREFIX, workspaceEmailIdempotencyKey() (+79 more)

### Community 11 - "app.config.ts"
Cohesion: 0.09
Nodes (57): argumentsSet, production, AiProviderConfig, EmbeddingConfig, parseAiConfig(), provider(), LOG_LEVELS, LogLevel (+49 more)

### Community 12 - "NotesService"
Cohesion: 0.07
Nodes (8): decodeVersionCursor(), encodeVersionCursor(), NotesService, Injectable, buildFolderSubrouter(), buildNoteSubrouter(), activeWorkspaceId(), createNote()

### Community 13 - "task.schema.ts"
Cohesion: 0.03
Nodes (71): ColorField(), attachmentFilterSchema, createAttachmentIntentSchema, updateAttachmentIntentSchema, createUserProfileSchema, updateUserProfileSchema, userProfileFilterSchema, sortDirectionSchema (+63 more)

### Community 14 - "export-generation.service.ts"
Cohesion: 0.13
Nodes (18): renderDocx(), bodyXml(), OPTIONS, partsOf(), source(), SUBJECT, ExportGenerationService, OPTIONS (+10 more)

### Community 15 - "CustomImage.ts"
Cohesion: 0.05
Nodes (55): created, Options, stubReducedMotion(), AttachmentUploadRequest, applyImageLayout(), CAPTION_SWALLOWED_EVENTS, Commands, createHandle() (+47 more)

### Community 16 - "page-geometry.ts"
Cohesion: 0.07
Nodes (52): canStepZoom(), clampMargins(), clampScale(), cssLength(), DEFAULT_PAGE_MARGINS, DEFAULT_ZOOM, exactPx(), isValidMargin() (+44 more)

### Community 17 - "openapi.builder.ts"
Cohesion: 0.07
Nodes (48): target, applyGlobalPrefix(), assertDeclaredSchemas(), buildOpenApiDocument(), buildSchemaCatalog(), collectControllers(), collectRefs(), componentName() (+40 more)

### Community 18 - "editor-harness.tsx"
Cohesion: 0.04
Nodes (39): attachmentDocument(), attachmentNode(), requests, collaborativeEditor(), EMPTY_DOCUMENT, NESTED_TASKS, typedKeys(), typeInEmptyNote() (+31 more)

### Community 19 - "docx.ts"
Cohesion: 0.13
Nodes (36): alignmentOf(), ALIGNMENTS, attachmentBlocks(), attrsOf(), Block, blocksFor(), blocksForAll(), cellSpan() (+28 more)

### Community 20 - ".run"
Cohesion: 0.10
Nodes (10): AuthorizationAdaptersService, Injectable, AuthorizedOperation, additionalAuthenticatedData(), DELIVERY_ERROR_CODES, hostOf(), SUBSCRIBABLE_EVENTS, Injectable (+2 more)

### Community 21 - "getRequestId"
Cohesion: 0.16
Nodes (17): ApiController, ApiRootResponse, Controller, Get, Req, getRequestId(), MembershipsController, routeUuid() (+9 more)

### Community 22 - "presence-store.ts"
Cohesion: 0.21
Nodes (17): applyPresenceJoined(), applyPresenceLeft(), byPresenceId(), EMPTY_PRESENCE_ROSTER, freezeRoster(), getPresenceRoster(), listenersByNote, PRESENCE_ROSTER_MAX (+9 more)

### Community 23 - "attachments.controller.test.ts"
Cohesion: 0.06
Nodes (35): applyContentHeaders(), AttachmentsController, contentDisposition(), INLINE_SERVABLE_MIME_TYPES, invalidRequest(), matchesEtag(), principalOf(), content() (+27 more)

### Community 24 - "email-renderer.service.ts"
Cohesion: 0.05
Nodes (64): AuthEmailQueueHandler, Injectable, brandingSettingsSchema, BrandingWorkspaceRow, DEFAULT_ACCENT_COLOR, EmailBranding, PLATFORM_BRANDING_NAME, resolveBranding() (+56 more)

### Community 25 - "auth.module.ts"
Cohesion: 0.08
Nodes (55): AppModule, Module, AttachmentsModule, Module, AuthModule, Module, AuthorizationModule, Module (+47 more)

### Community 26 - "webhooks/requests.ts"
Cohesion: 0.09
Nodes (41): create, deliveries, delivery, list, remove, retry, update, verify (+33 more)

### Community 27 - "markdown.ts"
Cohesion: 0.21
Nodes (28): applyMark(), cellText(), childNodes(), cleanedText(), codeBlockText(), collapseCell(), escapeHref(), escapeLineStart() (+20 more)

### Community 28 - "redirects.ts"
Cohesion: 0.09
Nodes (30): ForgotPasswordPage(), LoginPage(), MagicLinkResultPage(), RegisterPage(), ResetPasswordPage(), TwoFactorPage(), VerifyEmailPage(), SecuritySettingsPage() (+22 more)

### Community 29 - "config.module.ts"
Cohesion: 0.05
Nodes (37): AI_CONFIG, AiConfigProvider, Injectable, AppConfigProvider, Injectable, AuthConfigProvider, Injectable, ConfigModule (+29 more)

### Community 30 - "file-signature.ts"
Cohesion: 0.07
Nodes (29): EXTENSION_BY_TYPE, FILE_SIGNATURE_HEAD_BYTES, GZIP_SIGNATURE, hasOoxmlMarker(), isTarArchive(), isZipContainer(), OOXML_MARKER, PDF_SIGNATURE (+21 more)

### Community 31 - "realtime.gateway.ts"
Cohesion: 0.09
Nodes (32): toWebHeadersFromRaw(), binary(), createNoteCollaborationAwarenessSchema(), createNoteCollaborationSyncSchema(), createNoteCollaborationUpdateSchema(), id, NOTE_COLLABORATION_SCHEMA_VERSION, noteSelector (+24 more)

### Community 32 - "auth/requests.ts"
Cohesion: 0.07
Nodes (51): capabilities, refresh, replace, AdvancedSignInMethods(), AdvancedSignInMethodsProps, LoginForm(), refresh, replace (+43 more)

### Community 33 - "image-transfer.ts"
Cohesion: 0.11
Nodes (17): createImageTransferPlugin(), dataTransferOf(), setDropActive(), createObjectUrlRegistry(), DataTransferItemLike, DataTransferLike, hasImageFiles(), hasMeaningfulHtml() (+9 more)

### Community 34 - "workspace.schema.ts"
Cohesion: 0.04
Nodes (55): AcceptWorkspaceInvitationInput, acceptWorkspaceInvitationSchema, ChangeWorkspaceMemberRoleInput, changeWorkspaceMemberRoleSchema, CreateWorkspaceInput, createWorkspaceSchema, InvitationListQueryInput, invitationListQuerySchema (+47 more)

### Community 35 - "keyboard-shortcuts.ts"
Cohesion: 0.09
Nodes (40): areDocumentsEquivalent(), stableStringify(), stableValue(), accessibleNameFor(), EditorShortcutHandlerMap, EditorShortcuts, EditorShortcutsOptions, APPLE_KEY_NAMES (+32 more)

### Community 36 - "workspaces.service.ts"
Cohesion: 0.04
Nodes (75): authenticatedProcedure, commentSelectorSchema, CommentsTrpcRouter, CommentSubrouter, noteSelectorSchema, ADR-0002, Injectable, authenticatedProcedure (+67 more)

### Community 37 - "server-notes.ts"
Cohesion: 0.07
Nodes (47): DashboardLayout(), DashboardPage(), StandaloneNotePage(), NotesPage(), PinnedNotesPage(), RecentNotesPage(), browser, params (+39 more)

### Community 38 - "rate-limit.guard.test.ts"
Cohesion: 0.06
Nodes (35): AuthRateLimitMiddleware, Inject, Injectable, Bucket, InMemoryRateLimitStore, InMemoryRateLimitStoreOptions, policy, Injectable (+27 more)

### Community 39 - "TasksService"
Cohesion: 0.12
Nodes (3): TasksService, Injectable, buildTaskSubrouter()

### Community 40 - "toolbar-commands.ts"
Cohesion: 0.07
Nodes (49): ALLOWED_COLORS, isAllowedEditorColor(), NOTE_HIGHLIGHT_COLORS, NOTE_TEXT_COLORS, EditorToolbar(), EditorToolbarProps, CODE_BLOCK_LANGUAGE_OPTIONS, CodeBlockLanguageOption (+41 more)

### Community 41 - "whereWorkspace"
Cohesion: 0.16
Nodes (4): MembershipsService, normalizeEmail(), Injectable, whereWorkspace()

### Community 42 - "image-processing.service.ts"
Cohesion: 0.07
Nodes (41): sharp, AttachmentObjectVariant, ATTACHMENT_PROCESSING_ERRORS, ImageProcessingError, ImageProcessingRequest, ImageProcessor, PassthroughImageProcessor, ProcessedImage (+33 more)

### Community 43 - "autosave-machine.ts"
Cohesion: 0.07
Nodes (44): SaveStatusIndicator(), SaveStatusIndicatorProps, SEVERITY_CLASS, mocks, mount(), clearTimer(), NoteAutosaveHandle, TimerRef (+36 more)

### Community 44 - "comment.schema.ts"
Cohesion: 0.08
Nodes (28): COMMENT_ANCHOR_POSITION_MAX, COMMENT_ANCHOR_QUOTE_MAX_LENGTH, COMMENT_ANCHOR_RELATIVE_MAX_LENGTH, COMMENT_ANCHOR_SCHEMA_VERSION, COMMENT_ANCHOR_SCHEME_ABSOLUTE, COMMENT_ANCHOR_SCHEME_YJS, COMMENT_CONTENT_MAX_LENGTH, CommentAnchorInput (+20 more)

### Community 45 - "export-formats.spec.ts"
Cohesion: 0.13
Nodes (19): Account, apiGet(), apiPatch(), apiPost(), ATTACHMENT_BYTES, downloadBytes(), expectOk(), ExportManifest (+11 more)

### Community 46 - "createTenantContext"
Cohesion: 0.04
Nodes (61): mockEntry(), authorization(), harness(), runInTenant(), fakeAuthorization(), build(), build(), inWorkspace() (+53 more)

### Community 47 - "api-key-auth.service.ts"
Cohesion: 0.08
Nodes (20): ApiKeyAuthService, bearerSecret(), invalidApiKey(), KeyRow, liveRow, Injectable, API_KEY_ACTOR, ApiKeyRequest (+12 more)

### Community 48 - "server-workspaces.ts"
Cohesion: 0.05
Nodes (46): requestedPage(), items, WorkspacesPage(), WorkspacesPageProps, formatDate(), { notFound }, renderPage(), WorkspaceOverviewPage() (+38 more)

### Community 49 - "search.schema.ts"
Cohesion: 0.12
Nodes (21): RecentSearchesPayloadOutput, recentSearchesPayloadSchema, RecentSearchOutput, recentSearchSchema, SearchAvailabilityOutput, searchAvailabilitySchema, SearchHighlightOutput, searchHighlightSchema (+13 more)

### Community 50 - "GlobalSearchDialog.tsx"
Cohesion: 0.05
Nodes (58): browserStorage(), formatDate(), GlobalSearchDialog(), HighlightedText(), Recents(), SearchRequestError, statusMessage(), mocks (+50 more)

### Community 51 - "TiptapEditor.tsx"
Cohesion: 0.07
Nodes (53): AttachmentDirectory, isAllowedDocumentLink(), NoteDocumentHtmlResult, noteDocumentToSafeHtml(), prepareNoteDocumentForEditor(), createNoteLowlight(), CommentAnchorTarget, makeEditor() (+45 more)

### Community 52 - "image-upload-placeholder.ts"
Cohesion: 0.15
Nodes (24): decoration(), applyAction(), button(), createDecoration(), createImageInsertionController(), createImageUploadPlaceholderPlugin(), createPlaceholderDom(), DOM_CACHE (+16 more)

### Community 53 - "1. Workspace-scoping strategy"
Cohesion: 0.05
Nodes (38): ADR 0003: Make Better Auth the authentication and session authority, Alternatives considered, Consequences, Context, Decision, Migration and rollback impact, ADR 0006: Use durable job intent with idempotent BullMQ workers, Alternatives considered (+30 more)

### Community 54 - "storage-maintenance.service.ts"
Cohesion: 0.11
Nodes (35): STORAGE_MAINTENANCE_AUDIT_ACTION, STORAGE_MAINTENANCE_AUDIT_ENTITY_TYPE, STORAGE_MAINTENANCE_NOTES, AbandonedUploadCandidate, AbandonedUploadDecision, AttachmentLifecycleStatus, decideAbandonedUpload(), decideExportSweep() (+27 more)

### Community 55 - "notes.service.ts"
Cohesion: 0.05
Nodes (57): auditLogs, noteShares, projectAccess, INVITATION_EMAIL_IDEMPOTENCY_PREFIX, INVITATION_EMAIL_JOB_TYPE, INVITATION_EMAIL_PAYLOAD_VERSION, INVITATION_EMAIL_QUEUE_NAME, INVITATION_EMAIL_TEMPLATE_KEY (+49 more)

### Community 56 - "note-export-source.service.ts"
Cohesion: 0.07
Nodes (35): OBJECT_BEARING_VARIANTS, documentToMarkdown(), body(), doc(), OPTIONS, source(), text(), DEFAULT_ZIP_BOUNDS (+27 more)

### Community 57 - "dev-tooling.mjs"
Cohesion: 0.11
Nodes (40): assertLocalDockerEndpoint(), assertResetEnvironment(), assertResetTarget(), checkEnvironment(), composeArguments(), composeFile, E2E_ONE_SHOT_SERVICES, E2E_PERSISTENT_SERVICES (+32 more)

### Community 58 - "AttachmentDialogs.tsx"
Cohesion: 0.14
Nodes (23): AttachmentEntry, AttachmentResolution, documentHasAttachment(), documentHasImage(), documentHasNodeType(), attachmentDetail(), AttachmentDialogs(), AttachmentDialogsProps (+15 more)

### Community 59 - "shared-types/src/index.ts"
Cohesion: 0.07
Nodes (55): ADR-0001, ProjectMember, NotificationEmailPreference, NotificationKind, NotificationPage, NotificationReadResult, NotificationsMarkAllResult, NotificationTargetType (+47 more)

### Community 60 - ".transaction"
Cohesion: 0.10
Nodes (9): ApiKeysService, Injectable, AttachmentsService, Injectable, assertIdempotencyPayload(), createApiIdempotencyIdentity(), loadApiIdempotency(), lockApiIdempotency() (+1 more)

### Community 61 - "image-uploads.ts"
Cohesion: 0.05
Nodes (39): createAttachmentDirectory(), ImageInsertionController, deleteMock, fileMedia(), media(), setup(), uploadMock, attachmentNodeName() (+31 more)

### Community 62 - "tags.service.ts"
Cohesion: 0.09
Nodes (27): hashApiPayload(), TAG_AUDIT_ENTITY_TYPE, TAG_DOMAIN_EVENT_IDEMPOTENCY_PREFIX, TAG_DOMAIN_EVENT_PAYLOAD_VERSION, TAG_DOMAIN_EVENT_QUEUE, TAG_DOMAIN_EVENTS, TAG_MAX_PER_WORKSPACE, TAG_NAME_UNIQUE_CONSTRAINT (+19 more)

### Community 64 - "dependencies"
Cohesion: 0.05
Nodes (37): dependencies, clsx, @dnd-kit/core, @dnd-kit/utilities, pdfjs-dist, @radix-ui/react-slot, tailwind-merge, @tiptap/extension-collaboration (+29 more)

### Community 65 - "tasks.service.ts"
Cohesion: 0.03
Nodes (102): attachments, comments, folders, noteVersions, notes, projects, noteTags, tags (+94 more)

### Community 66 - "tenant/index.ts"
Cohesion: 0.04
Nodes (77): CommentRow, CommentScopedInput, CreateCommentServiceInput, DeleteCommentServiceInput, ListCommentsServiceInput, resolvers, SetCommentResolutionServiceInput, anchor (+69 more)

### Community 67 - "PageContainer.tsx"
Cohesion: 0.08
Nodes (28): mocks, PageContainer(), PageContainerProps, parseZoomOption(), zoomOptionValue(), PagePrintStyle(), PagePrintStyleProps, documentRoot() (+20 more)

### Community 68 - "scripts"
Cohesion: 0.06
Nodes (35): scripts, audit:prod, build, build:packages, clean, clean:deps, db:check, db:generate (+27 more)

### Community 69 - "note.ts"
Cohesion: 0.07
Nodes (38): FolderId, NoteId, Progress, ProjectId, FolderCreateResult, FolderDeleteResult, FolderPage, FolderUpdateResult (+30 more)

### Community 70 - "AuthController"
Cohesion: 0.09
Nodes (17): AuthController, Controller, Delete, Get, HttpCode, Param, Post, Req (+9 more)

### Community 71 - "compilerOptions"
Cohesion: 0.06
Nodes (30): ClipboardEventStub, compilerOptions, baseUrl, isolatedModules, jsx, lib, module, moduleResolution (+22 more)

### Community 72 - "public-environment.ts"
Cohesion: 0.08
Nodes (30): validateEnvironment(), AcceptWorkspaceInvitation(), ALLOWED_PROTOCOLS, DEVELOPMENT_DEFAULTS, parsePublicEnvironment(), PUBLIC_ENVIRONMENT_KEYS, PublicEnvironment, PublicEnvironmentInput (+22 more)

### Community 73 - "DashboardShell.tsx"
Cohesion: 0.18
Nodes (13): LogoutButton(), Breadcrumb(), BreadcrumbItem, breadcrumbsFor(), DashboardShell(), NotificationCenter(), TopBar(), ReactQueryProvider() (+5 more)

### Community 74 - "devDependencies"
Cohesion: 0.06
Nodes (33): devDependencies, jsdom, @next/env, @playwright/test, postcss, tailwindcss, @tailwindcss/postcss, @testing-library/jest-dom (+25 more)

### Community 75 - "tasks/index.ts"
Cohesion: 0.15
Nodes (17): addMonthsClamped(), advance(), advanceFixed(), assertCron(), invalidCron(), nextOccurrence(), next(), TASK_AUDIT_ENTITY_TYPE (+9 more)

### Community 76 - "image-processing.service.test.ts"
Cohesion: 0.10
Nodes (36): decodeHeicToJpeg(), HeicConverter, HeicDecodeOptions, isHeicDecoderAvailable(), resetHeicConverter(), setHeicConverter(), OPTIONS, ADR-0008 (+28 more)

### Community 77 - "queue-worker-processor.service.ts"
Cohesion: 0.14
Nodes (13): isRegisteredOutboxRoute(), QueueFailureCode, QueueRuntimeError, QueueWorkerInvocation, parseRows(), QueueOutboxRepository, Injectable, deadLetterRecordSchema (+5 more)

### Community 78 - "image-signature.ts"
Cohesion: 0.11
Nodes (19): AVIF_BRANDS, brandAt(), HEIF_BRANDS, JPEG_SIGNATURE, PNG_SIGNATURE, SNIFFED_IMAGE_TYPES, sniffImageMediaType(), sniffIsoBaseMedia() (+11 more)

### Community 79 - "ExportService"
Cohesion: 0.21
Nodes (3): ExportQueryRunner, ExportService, Injectable

### Community 80 - "hybrid-search.service.ts"
Cohesion: 0.10
Nodes (20): HYBRID_CANDIDATE_MULTIPLIER, HYBRID_MAX_CANDIDATES, HYBRID_SEARCH_CLOCK, HybridSearchClock, HybridSearchService, candidate(), fact(), input (+12 more)

### Community 81 - "dependencies"
Cohesion: 0.05
Nodes (37): dependencies, better-auth, @bull-board/api, @bull-board/express, bullmq, busboy, docx, ioredis (+29 more)

### Community 82 - "Part 62 — Export job lifecycle"
Cohesion: 0.16
Nodes (8): Deviations, Fixed after review, Implemented Work, Objective, Open risks and follow-ups, Part 62 — Export job lifecycle, Status, Verification

### Community 83 - "RequireAuthorization"
Cohesion: 0.07
Nodes (38): RequireAuthorization(), commentAuthorization(), CommentsController, noteAuthorization(), routeUuid(), Body, Controller, Delete (+30 more)

### Community 84 - "RedisService"
Cohesion: 0.11
Nodes (7): BetterAuthRedisStorage, secondsToMilliseconds(), Injectable, RedisService, Inject, Injectable, Inject

### Community 85 - "NoteExportSourceService"
Cohesion: 0.25
Nodes (3): Inject, NoteExportSourceService, Injectable

### Community 86 - "editor-tables.test.tsx"
Cohesion: 0.12
Nodes (26): caretInCell(), cell(), cellAttrs(), cellPosition(), cellPositions(), cellTexts(), columnCount(), EMPTY_DOCUMENT (+18 more)

### Community 87 - "devDependencies"
Cohesion: 0.06
Nodes (35): devDependencies, drizzle-kit, @nestjs/cli, socket.io-client, supertest, tsx, @types/busboy, @types/compression (+27 more)

### Community 88 - "meilisearch.service.ts"
Cohesion: 0.08
Nodes (21): MEILISEARCH_CONFIG, MeilisearchConfig, ImportedMeilisearchModule, hasProviderCode(), MeilisearchService, parseSearchHit(), parseSearchResponse(), safeMeilisearchError() (+13 more)

### Community 89 - "pdf-preview.ts"
Cohesion: 0.09
Nodes (17): configureWorker(), fetchPdfBytes(), MAX_PDF_PREVIEW_BYTES, MAX_PDF_PREVIEW_WIDTH, openPdfPreview(), PDF_PREVIEW_MESSAGES, PdfBytesResult, PdfDocument (+9 more)

### Community 90 - "devDependencies"
Cohesion: 0.06
Nodes (31): @darraghor/eslint-plugin-nestjs-typed, eslint, eslint-config-prettier, @eslint/js, eslint-plugin-import-x, eslint-plugin-jsx-a11y, husky, lint-staged (+23 more)

### Community 91 - "shared-validators/package.json"
Cohesion: 0.06
Nodes (30): dependencies, zod, description, devDependencies, typescript, vitest, exports, ./print.css (+22 more)

### Community 92 - "auth.schema.ts"
Cohesion: 0.09
Nodes (26): authPasswordSchema, CreateUserProfileInput, emailSchema, OAuthProviderIdInput, oauthProviderIdSchema, optionalCallbackUrlSchema, passkeyNameSchema, profileFields (+18 more)

### Community 94 - "DatabaseService"
Cohesion: 0.03
Nodes (66): Inject, Inject, Inject, AuthorizationEntryService, Injectable, SECURITY_CONFIG, SecurityConfig, StorageConfig (+58 more)

### Community 95 - "completed-parts/README.md"
Cohesion: 0.14
Nodes (12): Completed Parts, Cross-cutting records, Index, Naming and Scope, Required Workflow, Status Values, ADR 0012: Shared package charters cover runtime helpers and rendering assets, Alternatives considered (+4 more)

### Community 96 - "project.schema.ts"
Cohesion: 0.10
Nodes (25): CreateProjectInput, createProjectSchema, projectAccessRoleSchema, projectColorSchema, projectCoverImageUrlSchema, projectCreateResultSchema, projectDeleteResultSchema, projectDescriptionSchema (+17 more)

### Community 97 - "NoteIndexRepository"
Cohesion: 0.07
Nodes (29): main(), parseSearchReindexArguments(), renderSearchReindexResult(), SearchReindexCliError, SearchReindexSelection, CommonModule, Global, Module (+21 more)

### Community 98 - "auth-email-producer.service.ts"
Cohesion: 0.14
Nodes (15): additionalAuthenticatedData(), AuthEmailContext, AuthEmailEncryptionMetadata, AuthEmailEncryptionService, EncryptedAuthEmailContext, config, Inject, Injectable (+7 more)

### Community 99 - "embedding-reindex.ts"
Cohesion: 0.50
Nodes (4): main(), parseEmbeddingReindexArguments(), EmbeddingReindexCliModule, Module

### Community 100 - "note-index.repository.ts"
Cohesion: 0.08
Nodes (31): NOTE_INDEX_NAME, NOTE_INDEX_PRIMARY_KEY, NOTE_INDEX_SETTINGS, NOTE_INDEX_VERSION, noteIndexUid(), appendTimestampRange(), buildSearchFilterExpression(), buildSearchSortArray() (+23 more)

### Community 101 - "shared-types/package.json"
Cohesion: 0.08
Nodes (25): description, devDependencies, typescript, vitest, exports, files, dist, typescript (+17 more)

### Community 102 - "What You Must Do When Invoked"
Cohesion: 0.08
Nodes (24): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+16 more)

### Community 103 - "Part 42 — Editor image insertion"
Cohesion: 0.08
Nodes (24): Accessibility Notes, API, Configuration, and Operational Changes, Contract (additive, `packages/shared-validators/src/document.schema.ts`), Database and Data Changes, Editor, Files and Components, Handoff Notes, How `Plan.md`'s Verify clause is covered (+16 more)

### Community 104 - "What You Must Do When Invoked"
Cohesion: 0.08
Nodes (24): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+16 more)

### Community 105 - "compilerOptions"
Cohesion: 0.08
Nodes (24): compilerOptions, allowSyntheticDefaultImports, composite, esModuleInterop, forceConsistentCasingInFileNames, isolatedModules, lib, module (+16 more)

### Community 106 - "NoteProjectionRepository"
Cohesion: 0.15
Nodes (10): NoteIndexDocument, parseDocuments(), clampPageSize(), dedupeAndCapTags(), NoteProjectionPage, NoteProjectionRepository, truncate(), Injectable (+2 more)

### Community 107 - "CommentsService"
Cohesion: 0.23
Nodes (5): CommentQueryRunner, CommentsService, isRecord(), Injectable, buildCommentSubrouter()

### Community 108 - "QueueInfrastructureService"
Cohesion: 0.11
Nodes (9): createPool(), QueueInfrastructureService, Inject, Injectable, QueueLifecycleService, Inject, Injectable, QueueReadinessIndicator (+1 more)

### Community 109 - "TagsController"
Cohesion: 0.15
Nodes (13): routeUuid(), tagAuthorization(), TagsController, Body, Controller, Delete, Get, HttpCode (+5 more)

### Community 110 - "IsoTimestamp"
Cohesion: 0.14
Nodes (28): API_KEY_SECRET_PREFIX, ApiKeyCreateResult, ApiKeyListQuery, ApiKeyPage, ApiKeyRevokeResult, ApiKeyScope, ApiKeySortField, ApiKeySummary (+20 more)

### Community 111 - "task.ts"
Cohesion: 0.13
Nodes (19): TaskId, TaskStatusId, CustomTaskStatusList, TaskBulkResult, TaskBulkSkip, TaskBulkSkipReason, TaskCreateResult, TaskDeleteResult (+11 more)

### Community 112 - "authorization-entry.service.ts"
Cohesion: 0.04
Nodes (83): harness(), noteFacts(), NOW, principal, TransportAuthorizationInput, actorFromPrincipal(), ApiKeyAuthorizationActor, ApiKeyScope (+75 more)

### Community 113 - "note-images.spec.ts"
Cohesion: 0.14
Nodes (21): apiPost(), BLUE_PNG, chunk(), crc32(), CRC_TABLE, createNote(), createWorkspace(), GREEN_PNG (+13 more)

### Community 114 - "task-list.spec.ts"
Cohesion: 0.10
Nodes (11): Account, apiGet(), inviteAndJoin(), ListPage, listTasks(), MemberRow, NoteRow, register() (+3 more)

### Community 115 - "StorageMaintenanceService"
Cohesion: 0.21
Nodes (4): attachmentObjectKeys(), SweepAccumulator, StorageMaintenanceService, Injectable

### Community 116 - "export.ts"
Cohesion: 0.20
Nodes (10): ExportCancelResult, ExportFormat, ExportOptions, ExportPage, ExportSource, ExportStatus, SUPPORTED_EXPORT_FORMATS, SUPPORTED_EXPORT_SOURCES (+2 more)

### Community 117 - "DatabaseTransaction"
Cohesion: 0.10
Nodes (20): DatabaseTransaction, FakeTx, TaskStatusesService, Injectable, FakeTx, scopedDatabase(), deterministicCounts(), runWorker() (+12 more)

### Community 118 - "compilerOptions"
Cohesion: 0.09
Nodes (22): compilerOptions, emitDecoratorMetadata, experimentalDecorators, jsx, lib, module, moduleResolution, outDir (+14 more)

### Community 119 - "task-views.spec.ts"
Cohesion: 0.11
Nodes (12): Account, apiPost(), createTask(), dayKeyInZone(), NoteRow, register(), selectView(), signIn() (+4 more)

### Community 120 - "README.md"
Cohesion: 0.12
Nodes (11): Part Completion Checklist, 1. Inventory and stop writers, 2. Back up the legacy volumes, 3. Choose recovery, do not merge blindly, 4. Verify and retain rollback, Legacy development volume recovery, Disposable end-to-end stack, Local resource budget (+3 more)

### Community 121 - "storage/index.ts"
Cohesion: 0.32
Nodes (11): StoragePlanDefaults, buildWorkspaceStorageUsage(), fitsWithinQuota(), QUOTA_CHARGED_STATUSES, QUOTA_RESERVED_STATUSES, resolveEffectiveLimitBytes(), safeBytes(), ReadWorkspaceStorageUsageInput (+3 more)

### Community 122 - "common.schema.ts"
Cohesion: 0.04
Nodes (59): API_KEY_PREFIX_LENGTH, API_KEY_SECRET_PATTERN, API_KEY_SECRET_PREFIX, apiKeyCreateResultSchema, ApiKeyListQueryInput, apiKeyListQuerySchema, apiKeyNameSchema, apiKeyPageSchema (+51 more)

### Community 123 - "better-auth.setup.ts"
Cohesion: 0.14
Nodes (21): betterAuthCookieAttributes(), configuredSocialProviders(), createBetterAuthNodeHandler(), isNonRememberedSession(), passkeyPluginOptions(), preserveNonRememberedRotationExpiry(), RECENT_AUTHENTICATION_PATHS, requestCorrelationId() (+13 more)

### Community 124 - "Part 48 — Add board, calendar, and progress views"
Cohesion: 0.10
Nodes (20): 48.1 — Custom task status CRUD, 48.2 — Board, 48.3 — Calendar, 48.4 — Progress, 48.5 — My Tasks widget, 48.6 — Part 47's recorded editor-affordance defect, closed, 48.7 — Routes and view persistence, API, Configuration, and Operational Changes (+12 more)

### Community 125 - "scripts"
Cohesion: 0.10
Nodes (21): scripts, build, db:check, db:generate, db:migrate, db:seed, db:studio, dev (+13 more)

### Community 126 - "scripts"
Cohesion: 0.11
Nodes (19): scripts, build, contracts:build, dev, env:validate, format, format:check, lint (+11 more)

### Community 127 - "WebhooksController"
Cohesion: 0.16
Nodes (14): RateLimitTier(), routeUuid(), Body, Controller, Delete, Get, HttpCode, Patch (+6 more)

### Community 128 - "queue-infrastructure.service.ts"
Cohesion: 0.15
Nodes (17): frozenWorker(), QUEUE_CONFIG, QueueConfig, QueueConfigProvider, QueueWorkerConfig, Injectable, BullJobEnvelope, QueueRouteMetadata (+9 more)

### Community 129 - "webhooks/index.ts"
Cohesion: 0.16
Nodes (22): ScheduleWebhookDeliveriesInput, ScheduleWebhookReplayInput, WEBHOOK_DELIVER_IDEMPOTENCY_PREFIX, webhookDeliverIdempotencyKey(), webhookRetryIdempotencyKey(), canonicalString(), HEADER_PATTERN, signatureHeader() (+14 more)

### Community 130 - "Part 43 — Add image manipulation UI"
Cohesion: 0.11
Nodes (18): Accessibility Notes, API, Configuration, and Operational Changes, Chrome (React, portalled past the paper's transform), Contract (additive, `packages/shared-validators/src/document.schema.ts`), Database and Data Changes, Files and Components, Handoff Notes, Implemented Work (+10 more)

### Community 131 - "Part 45 — Add storage quotas and cleanup"
Cohesion: 0.11
Nodes (19): API, Configuration, and Operational Changes, Cleanup (`apps/api/src/maintenance/`), Configuration and tooling, Database and Data Changes, Derived usage accounting (`apps/api/src/storage/`), Files and Components, Frontend (`apps/web`), Handoff Notes (+11 more)

### Community 132 - "attachments.service.ts"
Cohesion: 0.06
Nodes (54): AdmissionRejection, AdmissionResult, AdmittedFileUpload, AdmittedImageUpload, AdmittedUpload, admitUpload(), ATTACHMENT_SIGNATURE_HEAD_BYTES, PDF (+46 more)

### Community 133 - "shell/requests.ts"
Cohesion: 0.12
Nodes (23): noteNavigation, shell, tagNavigation, alpha, beta, WorkspaceSwitcher(), load, save (+15 more)

### Community 134 - "TasksController"
Cohesion: 0.17
Nodes (13): routeUuid(), taskAuthorization(), TasksController, Body, Controller, Delete, Get, HttpCode (+5 more)

### Community 135 - "TaskStatusesController"
Cohesion: 0.18
Nodes (10): TaskStatusesController, Body, Controller, Delete, Get, HttpCode, Patch, Post (+2 more)

### Community 136 - "extensions/index.ts"
Cohesion: 0.06
Nodes (66): node(), paint(), ATTACHMENT_KIND_LABELS, attachmentIconKind, CODE_EXTENSIONS, createAttachmentIcon(), ICON_PATHS, SPREADSHEET_EXTENSIONS (+58 more)

### Community 137 - "components.json"
Cohesion: 0.11
Nodes (17): aliases, components, hooks, lib, ui, utils, iconLibrary, rsc (+9 more)

### Community 138 - "note-attachments.spec.ts"
Cohesion: 0.12
Nodes (9): apiPost(), BUNDLE_ZIP, createNote(), NOTES_TXT, PHOTO_PNG, register(), REPORT_PDF, storedDocument() (+1 more)

### Community 139 - "project-board.spec.ts"
Cohesion: 0.14
Nodes (12): Account, apiPost(), columnList(), columnName(), createProjectNote(), createUndatedProjectTask(), ProjectRow, register() (+4 more)

### Community 140 - "Part 41 — Implement image ingestion and processing"
Cohesion: 0.11
Nodes (18): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+10 more)

### Community 141 - "ProjectsController"
Cohesion: 0.21
Nodes (12): projectAuthorization(), ProjectsController, routeUuid(), Body, Controller, Delete, Get, HttpCode (+4 more)

### Community 142 - "Part 40 — Build secure object-storage services"
Cohesion: 0.12
Nodes (16): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Local-environment caveat for `pnpm build` (pre-existing, not a Parts 40–42 defect) (+8 more)

### Community 143 - "Part 44 — Implement generic attachment flows"
Cohesion: 0.12
Nodes (16): API (`apps/api`), API, Configuration, and Operational Changes, Database and Data Changes, Editor and frontend (`apps/web`), Files and Components, Handoff Notes, Implemented Work, Important Decisions (+8 more)

### Community 144 - "Part 47 — Implement standalone task CRUD and list view"
Cohesion: 0.12
Nodes (17): API, Configuration, and Operational Changes, Backend, Cross-cutting defect fixed en route, Database and Data Changes, Files and Components, Frontend, Handoff Notes, Implemented Work (+9 more)

### Community 145 - "Part 49 — Add project board and timeline views"
Cohesion: 0.12
Nodes (17): 49.1 — Note board columns, 49.2 — `NoteBoard.tsx`, 49.3 — `NoteTimeline.tsx` + `lib/notes/timeline.ts`, 49.4 — The stable view-switching contract, API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes (+9 more)

### Community 146 - "Core Features Specification"
Cohesion: 0.12
Nodes (17): 10. Real-Time Collaboration (Socket.io), 11. AI Features, 12. Authentication & Authorization, 13. Background Jobs (BullMQ), 14. Email System, 15. API & Webhooks, 16. White-Label & Customization, 1. White A4/Letter Page Layout (+9 more)

### Community 147 - "compilerOptions"
Cohesion: 0.12
Nodes (16): compilerOptions, composite, declaration, module, moduleResolution, noEmit, outDir, rootDir (+8 more)

### Community 148 - "compilerOptions"
Cohesion: 0.12
Nodes (16): compilerOptions, composite, declaration, module, moduleResolution, noEmit, outDir, rootDir (+8 more)

### Community 149 - "ImageToolbar.tsx"
Cohesion: 0.08
Nodes (18): Loading(), IMAGE_EXTENSION_NAME, ImageAttributePatch, updateSelectedImage(), ALIGN_CAPTIONS, ALIGN_LABELS, IMAGE_TOOLBAR_LABEL, ImageToolbar() (+10 more)

### Community 150 - "VersionHistory.tsx"
Cohesion: 0.11
Nodes (23): currentDocument, historical, refresh, requestNoteVersion, requestNoteVersions, restoreNoteVersion, summary, failureCopy() (+15 more)

### Community 151 - "filename.ts"
Cohesion: 0.23
Nodes (14): basename(), boundName(), canonicalDisplayExtension(), declaredFileExtension(), DISPLAY_EXTENSION_BY_TYPE, RESERVED_DEVICE_NAMES, sanitizeAttachmentFilename(), SanitizedAttachmentFilename (+6 more)

### Community 152 - "tags-templates.spec.ts"
Cohesion: 0.13
Nodes (5): inviteAndJoin(), ListPage, NoteRow, register(), TagRow

### Community 153 - "Coverage remediation — making `pnpm test:ci` pass"
Cohesion: 0.12
Nodes (16): Bug found and fixed, Cover the gaps, Coverage remediation — making `pnpm test:ci` pass, Database and Data Changes, Files and Components, Fix two pre-existing flaws the newly-running tests exposed, Handoff Notes, Implemented Work (+8 more)

### Community 154 - "Part 21 — Integrate Better Auth on the backend"
Cohesion: 0.13
Nodes (15): API, Configuration, and Operational Changes, Completion Verification Update, Database and Data Changes, Dependency Advisory Resolution, Files and Components, Handoff Notes, Implemented Work, Important Decisions (+7 more)

### Community 155 - "Part 46 — Implement tags and templates"
Cohesion: 0.12
Nodes (16): API, Configuration, and Operational Changes, Backend, Database and Data Changes, Files and Components, Frontend, Handoff Notes, Implemented Work, Important Decisions (+8 more)

### Community 156 - "attachment.ts"
Cohesion: 0.16
Nodes (15): AttachmentBlurPlaceholder, AttachmentDeleteResult, AttachmentDetail, AttachmentListResult, AttachmentMedia, AttachmentMediaType, AttachmentServableVariant, AttachmentStatus (+7 more)

### Community 157 - "ObjectStorageService"
Cohesion: 0.19
Nodes (7): ABSENT_OBJECT_CODES, BUCKET_ALREADY_OWNED_CODES, errorCode(), isAbsent(), ObjectStorageService, Injectable, bucket()

### Community 158 - "search.service.ts"
Cohesion: 0.09
Nodes (22): HIGHLIGHT_POST_TAG, HIGHLIGHT_PRE_TAG, NoteSearchCandidate, NoteSearchCandidatePage, logger, meilisearchWithSearch(), repository(), NoteSearchFact (+14 more)

### Community 159 - "NoteSharesController"
Cohesion: 0.22
Nodes (8): NoteSharesController, routeUuid(), Body, Controller, Delete, Get, Req, Put

### Community 160 - "Part 50 — Establish BullMQ queues and workers"
Cohesion: 0.22
Nodes (9): Architecture and security decisions, Commands, Corrective verification — 2026-08-14, Files, Known limitations and follow-up boundaries, Migration, Part 50 — Establish BullMQ queues and workers, Result (+1 more)

### Community 161 - "compilerOptions"
Cohesion: 0.13
Nodes (14): compilerOptions, declaration, noEmit, rootDir, sourceMap, tsBuildInfoFile, exclude, extends (+6 more)

### Community 162 - "Part 58 — Integrate Yjs collaborative editing"
Cohesion: 0.15
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 163 - "task-board.test.tsx"
Cohesion: 0.18
Nodes (11): alpha, beta, delta, epsilon, gamma, mocks, openBoard(), page (+3 more)

### Community 164 - "Part 04 — Scaffold the Next.js Web Application"
Cohesion: 0.13
Nodes (14): 2026-07-24 canonical stylesheet remediation, 2026-07-24 Phase 1 integration verification, 2026-07-27 final completion verification, Dependency and Build Decisions, Handoff Notes, Implemented Work, Important Boundaries, Known Limitations and Follow-up Work (+6 more)

### Community 165 - "Part 11 — Implement Configuration and Dependency Clients"
Cohesion: 0.13
Nodes (14): 2026-07-27 final completion verification, API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work (+6 more)

### Community 166 - "Part 22 — Build authentication screens and route protection"
Cohesion: 0.13
Nodes (14): API, Configuration, and Operational Changes, Completion Verification Update, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work (+6 more)

### Community 167 - "Part 23 — Add OAuth, two-factor authentication, passkeys, and session controls"
Cohesion: 0.13
Nodes (14): API, Configuration, and Operational Changes, Completion Verification Update, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work (+6 more)

### Community 168 - "Part NN — Part Title"
Cohesion: 0.13
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 169 - "webhook.schema.ts"
Cohesion: 0.09
Nodes (24): WEBHOOK_SECRET_PATTERN, WEBHOOK_SECRET_PREFIX, WebhookCreateInput, webhookCreateResultSchema, webhookCreateSchema, webhookDeleteResultSchema, webhookDeliveryErrorCodeSchema, WebhookDeliveryListQueryInput (+16 more)

### Community 170 - "Notted"
Cohesion: 0.13
Nodes (15): Architecture Principles, Codex commands, Completion Records, Contributing, Current Documentation, Deployment, Development Workflow, License (+7 more)

### Community 171 - "webhooks.service.test.ts"
Cohesion: 0.09
Nodes (15): Awaitable, byId, deliveryRow, dialect, HarnessOptions, outboxPayload, principal, resolveHost (+7 more)

### Community 172 - ".info"
Cohesion: 0.17
Nodes (5): CollaborationDb, NoteCollaborationRepository, Injectable, NoteCollaborationService, Injectable

### Community 173 - "hybrid-ranking.service.ts"
Cohesion: 0.14
Nodes (15): AuthoritativeRankingFact, finite(), HybridRankedCandidate, HybridRankingService, LEXICAL_WEIGHT, LexicalRankingCandidate, normalizeRelativeScores(), SEMANTIC_WEIGHT (+7 more)

### Community 174 - "Part 01 — Record architecture decisions and resolve specification gaps"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 175 - "Part 02 — Initialize the pnpm/Turborepo monorepo"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 176 - "Part 03 — Establish formatting, linting, and commit quality gates"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 177 - "Part 05 — Scaffold the NestJS API Application"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 178 - "Part 06 — Create Shared Types and Validators"
Cohesion: 0.15
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 179 - "Part 07 — Add continuous integration"
Cohesion: 0.12
Nodes (14): 2026-07-27 final completion verification, API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work (+6 more)

### Community 180 - "Part 08 — Define Environment Contracts"
Cohesion: 0.14
Nodes (14): 2026-07-27 final completion verification, API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work (+6 more)

### Community 181 - "Part 09 — Build the Development Compose Stack"
Cohesion: 0.14
Nodes (14): 2026-07-27 final completion verification, API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work (+6 more)

### Community 182 - "Part 10 — Add Developer Commands and Onboarding Documentation"
Cohesion: 0.14
Nodes (14): 2026-07-27 final completion verification, API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work (+6 more)

### Community 183 - "Part 12 — Configure Drizzle ORM and Migration Tooling"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 184 - "Part 13 — Implement Identity and Authentication Tables"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 185 - "Part 14 — Implement Workspace and Membership Tables"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 186 - "Part 15 — Implement Projects, Notes, Hierarchy, and Ordering"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 187 - "Part 18 — Operations and Integration Tables"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 188 - "Part 20 — Create realistic seed data"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 189 - "Part 26 — Implement workspace lifecycle APIs"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 190 - "Part 27 — Build workspace screens"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 191 - "Part 28 — Implement membership and invitation flows"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 192 - "Part 29 — Implement project CRUD APIs"
Cohesion: 0.15
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 193 - "Part 33 — Establish the TipTap document contract"
Cohesion: 0.15
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 194 - "Part 34 — Build the basic editor and toolbar"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 195 - "Part 35 — Add tables, checklists, markdown shortcuts, and block behavior"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 196 - "Part 36 — Build slash commands and mentions"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 197 - "Part 37 — Implement the A4/Letter page container"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 198 - "Part 38 — Add page breaks, focus mode, and print styling"
Cohesion: 0.14
Nodes (14): Amendment — 2026-08-16: `print.css` and page geometry relocated for Part 63, API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work (+6 more)

### Community 199 - "Part 39 — Implement reliable save behavior"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 200 - "Third-Party Notices"
Cohesion: 0.13
Nodes (12): 1. `libheif-js@1.19.8` — LGPL-3.0, 2. `@img/sharp-libvips-linux-x64@1.3.0` — LGPL-3.0-or-later, Copyleft components, Direct runtime dependencies added by Plan parts 40–44, Dual-licensed: `jszip@3.10.1` — `(MIT OR GPL-3.0-or-later)`, MIT elected (Part 64), Non-LGPL copyleft: `lightningcss` — MPL-2.0 (triaged, no obligation in the shipped product), Ongoing obligations, Re-auditing for new copyleft (+4 more)

### Community 201 - "Notted — Corporate Notes Platform"
Cohesion: 0.09
Nodes (22): AI Coding Conventions (CLAUDE.md), Available Make Commands, Deployment Instructions, Development Stack (`compose.yaml`), Development Steps, Development Workflow, Docker Compose Configuration, Environment Variables (+14 more)

### Community 202 - "WorkspacesService"
Cohesion: 0.13
Nodes (10): StorageQuotaService, Injectable, whereWorkspaceId(), isUniqueViolationOnConstraint(), knownSafePersistedSettings(), normalizeStoredSettings(), Injectable, WorkspacesService (+2 more)

### Community 203 - "NoteDetailView.tsx"
Cohesion: 0.08
Nodes (32): Sidebar(), ConvertNoteTypeControl(), note, NoteCard(), updatedLabel(), NoteDetailView(), NoteGrid(), NoteLifecycleActions() (+24 more)

### Community 205 - "All-in-Docker development stack"
Cohesion: 0.12
Nodes (13): 2026-08-04 registration and email-verification flow, 2026-08-04 runtime verification, All-in-Docker development stack, API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Implemented Work, Important Decisions (+5 more)

### Community 206 - "Part 16 — Implement Tags, Attachments, Comments, and Note Versions"
Cohesion: 0.15
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 207 - "Part 17 — Implement Standalone Task Data"
Cohesion: 0.15
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 208 - "Part 19 — Database-level tenant protection and retention policies"
Cohesion: 0.15
Nodes (12): ADR 0009, Files and components, Implemented work, Isolation coverage matrix, Known limitations and follow-up, Objective and scope, Part 19 — Database-level tenant protection and retention policies, Retention configuration (+4 more)

### Community 209 - "Part 30 — Build project list and detail screens"
Cohesion: 0.15
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 210 - "Part 31 — Implement core note APIs"
Cohesion: 0.15
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 211 - "Part 32 — Build note browsing and hierarchy UI"
Cohesion: 0.15
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 212 - "Part 51 — Meilisearch indexing pipeline"
Cohesion: 0.15
Nodes (12): Commands, Files and test source, Index and document contract, Known limitations, Objective and current result, Operational command, Part 51 — Meilisearch indexing pipeline, Prefix isolation, rollback, and rebuild (+4 more)

### Community 213 - "Local development"
Cohesion: 0.15
Nodes (13): Clone to running, Dashboard shell and notifications, Database workflow, Deterministic development seed, Host tooling and the debug-ports override, Infrastructure lifecycle, Local authentication and Mailpit, Local development (+5 more)

### Community 214 - "Database Schema (Drizzle ORM)"
Cohesion: 0.15
Nodes (13): API Keys Table, Attachments Table, Audit Logs Table, Comments Table, Database Schema (Drizzle ORM), Note Embeddings Table (Semantic Search), Note Versions Table, Notes Table (+5 more)

### Community 215 - "globalDependencies"
Cohesion: 0.15
Nodes (11): CI, eslint.config.mjs, NODE_ENV, .npmrc, pnpm-workspace.yaml, .prettierignore, .prettierrc.json, globalDependencies (+3 more)

### Community 216 - "comment.ts"
Cohesion: 0.20
Nodes (9): CommentAnchor, CommentAnchorScheme, CommentAuthor, CommentChangedEvent, CommentDeleteResult, CommentMutationResult, CommentPage, CommentSummary (+1 more)

### Community 217 - "RealtimeGateway"
Cohesion: 0.13
Nodes (8): RealtimeRoomSelector, RealtimeGateway, RealtimeSocket, Injectable, RealtimeRateLimitService, Injectable, WebSocketGateway, WebSocketServer

### Community 218 - "NoteEditorSurface.tsx"
Cohesion: 0.03
Nodes (73): clamp(), CommentAnchorRange, createCommentAnchor(), decodeRelative(), encodeRelative(), fromBase64Url(), readBinding(), resolveCommentAnchor() (+65 more)

### Community 219 - "WorkspacesController"
Cohesion: 0.22
Nodes (11): Body, Controller, Delete, Get, Patch, Post, Query, Req (+3 more)

### Community 220 - "notes/view-preference.ts"
Cohesion: 0.30
Nodes (7): isMode(), NoteViewMode, noteViewPreferenceKey(), PreferenceStorage, readNoteViewPreference(), VALUES, writeNoteViewPreference()

### Community 221 - "Notted Coding Conventions"
Cohesion: 0.15
Nodes (13): API design, Architecture, Code style, Database, Docker and operations, End-to-end runs (local resource budget), Frontend, graphify (+5 more)

### Community 222 - "Part 24 — Implement centralized authorization"
Cohesion: 0.17
Nodes (11): Authored verification (not run), Completion Verification Update, Database, API, and operational changes, Files and components, Important decisions, Known limitations and follow-up, Objective and implemented work, Part 24 — Implement centralized authorization (+3 more)

### Community 223 - "Decision"
Cohesion: 0.13
Nodes (15): ADR 0008: Runtime and package compatibility baseline, Alternatives considered, Consequences, Context, Decision, Development container baseline, Migration and rollback, Part 41 image-pipeline dependency review (+7 more)

### Community 224 - "package.json"
Cohesion: 0.25
Nodes (7): description, engines, pnpm, name, packageManager, private, version

### Community 225 - "inputs"
Cohesion: 0.39
Nodes (12): package.json, scripts/**/*, src/**/*, test/**/*, $TURBO_DEFAULT$, vitest.config.ts, inputs, tsconfig*.json (+4 more)

### Community 226 - "tasks"
Cohesion: 0.20
Nodes (10): cache, persistent, cache, cache, tasks, dev, format, format:check (+2 more)

### Community 227 - "print-export.spec.ts"
Cohesion: 0.18
Nodes (5): inflatedStreams(), pdfSource(), PRINT_HIDDEN_SELECTORS, register(), ADR-0008

### Community 228 - "ApiResponse"
Cohesion: 0.16
Nodes (19): Account, apiGet(), apiPatch(), apiPost(), expectOk(), provisionAccount(), signIn(), Account (+11 more)

### Community 230 - "shared-types/tsconfig.json"
Cohesion: 0.18
Nodes (10): compilerOptions, outDir, rootDir, exclude, extends, include, dist, node_modules (+2 more)

### Community 231 - "shared-validators/tsconfig.json"
Cohesion: 0.18
Nodes (10): compilerOptions, outDir, rootDir, exclude, extends, include, dist, node_modules (+2 more)

### Community 232 - "test:ci"
Cohesion: 0.17
Nodes (13): ^build, coverage/**, DATABASE_URL, test-results/**, test, test:ci, type-check, dependsOn (+5 more)

### Community 233 - "realtime.integration.test.ts"
Cohesion: 0.16
Nodes (9): collaborationSync(), collaborationUpdate(), identity(), join(), PresenceAck, presenceAnnounce(), SyncAck, verificationLink() (+1 more)

### Community 234 - "svg-safety.ts"
Cohesion: 0.26
Nodes (11): ALLOWED_DATA_URI_PREFIXES, FOREIGN_OBJECT, isAllowedReference(), refuse(), ROOT_ELEMENT, SAFE, scanSvgSource(), SCRIPT_ELEMENT (+3 more)

### Community 235 - "webhooks.e2e.test.ts"
Cohesion: 0.11
Nodes (15): allRoutes(), bearer(), call(), ENVIRONMENT_KEYS, EnvironmentSnapshot, freshPrincipal(), LOOPBACK(), Method (+7 more)

### Community 236 - "search.ts"
Cohesion: 0.14
Nodes (12): Paginated, APP_NAME, RecentSearch, RecentSearchesPayload, SEARCH_API_PATHS, SearchAvailability, SearchHighlight, SearchMode (+4 more)

### Community 237 - "Disposable end-to-end stack, and the production defect it uncovered"
Cohesion: 0.20
Nodes (10): Decisions worth keeping, Disposable end-to-end stack, and the production defect it uncovered, Files, Known gaps and follow-up, Test defects corrected, The production defect the clean database uncovered, Verification, What changed (+2 more)

### Community 238 - "Part 25 — Build the dashboard shell"
Cohesion: 0.20
Nodes (9): API and security boundaries, Authored verification (not run), Completion Verification Update, Database and migration, Known limitations and ownership handoff, Objective and implemented work, Part 25 — Build the dashboard shell, Revision history (+1 more)

### Community 239 - "Phase 2 — Local Infrastructure and Configuration"
Cohesion: 0.40
Nodes (5): Part 10 — Add developer commands and onboarding documentation, Part 11 — Implement configuration and dependency clients, Part 8 — Define environment contracts, Part 9 — Build the development Compose stack, Phase 2 — Local Infrastructure and Configuration

### Community 240 - "Phase 3 — Database Design and Tenant Safety"
Cohesion: 0.20
Nodes (10): Part 12 — Configure Drizzle ORM and migration tooling, Part 13 — Implement identity and authentication tables, Part 14 — Implement workspace and membership tables, Part 15 — Implement projects, notes, hierarchy, and ordering, Part 16 — Implement tags, attachments, comments, and versions, Part 17 — Implement standalone task data, Part 18 — Implement operations and integration tables, Part 19 — Add database-level tenant protection and retention policies (+2 more)

### Community 241 - ".prettierrc.json"
Cohesion: 0.20
Nodes (9): arrowParens, bracketSpacing, endOfLine, printWidth, semi, singleQuote, tabWidth, trailingComma (+1 more)

### Community 242 - "build"
Cohesion: 0.20
Nodes (10): !.next/cache/**, NEXT_PUBLIC_API_URL, NEXT_PUBLIC_APP_URL, NEXT_PUBLIC_WS_URL, dependsOn, env, outputs, dist/** (+2 more)

### Community 243 - "Notted Agent Instructions"
Cohesion: 0.22
Nodes (9): Authority, Automatic Skill and Agent Routing, graphify, Notted Agent Instructions, opencode Support, Project-Wide Rules, Required Part Workflow, Skills and Codex Agents (+1 more)

### Community 244 - "Part 59 — Presence and collaboration UI"
Cohesion: 0.15
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 245 - "Part 60 — Inline comments, mentions, and notifications"
Cohesion: 0.15
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 246 - "main.ts"
Cohesion: 0.06
Nodes (37): Inject, BETTER_AUTH_NODE_HANDLER, ApiErrorEnvelope, ApiExceptionFilter, ERROR_BY_STATUS, statusForUnknownException(), collectIssues(), validationExceptionFactory() (+29 more)

### Community 247 - "note-collaboration.projection.ts"
Cohesion: 0.17
Nodes (11): COMPACTION_MIN_BYTES, COMPACTION_MIN_UPDATES, CompactionInput, PROJECTION_DEBOUNCE_MS, PROJECTION_MAX_WAIT_MS, shouldCompact(), NoteCollaborationProjectionService, PendingProjection (+3 more)

### Community 248 - "graphify reference: extra exports and benchmark"
Cohesion: 0.22
Nodes (8): graphify reference: extra exports and benchmark, Step 6b - Wiki (only if --wiki flag), Step 7 - Neo4j export (only if --neo4j or --neo4j-push flag), Step 7a - FalkorDB export (only if --falkordb or --falkordb-push flag), Step 7b - SVG export (only if --svg flag), Step 7c - GraphML export (only if --graphml flag), Step 7d - MCP server (only if --mcp flag), Step 8 - Token reduction benchmark (only if total_words > 5000)

### Community 249 - "Deliver a Notted Plan Part"
Cohesion: 0.22
Nodes (7): Part Completion Checklist, Actions, Deliver a Notted Plan Part, Establish context, Execute, Synchronous Delegation Protocol in Claude Code, Verify and hand off

### Community 250 - "BrowserPoolService"
Cohesion: 0.22
Nodes (3): BrowserPoolService, Injectable, LaunchedBrowser

### Community 251 - "graphify reference: extra exports and benchmark"
Cohesion: 0.22
Nodes (8): graphify reference: extra exports and benchmark, Step 6b - Wiki (only if --wiki flag), Step 7 - Neo4j export (only if --neo4j or --neo4j-push flag), Step 7a - FalkorDB export (only if --falkordb or --falkordb-push flag), Step 7b - SVG export (only if --svg flag), Step 7c - GraphML export (only if --graphml flag), Step 7d - MCP server (only if --mcp flag), Step 8 - Token reduction benchmark (only if total_words > 5000)

### Community 252 - "export-pdf.integration.test.ts"
Cohesion: 0.12
Nodes (14): baseStyle(), buildStandaloneHtml(), escapeHtml(), printStylesheet(), requireFrom, DOCUMENT, puppeteerLauncherProvider, HAS_CHROMIUM (+6 more)

### Community 254 - "latestActionLink"
Cohesion: 0.08
Nodes (16): registerAndVerify(), clearMailpit(), isObject(), latestActionLink(), MailpitList, MailpitMessage, MailpitMessageSummary, parseList() (+8 more)

### Community 255 - "presence-client.ts"
Cohesion: 0.16
Nodes (17): asInteger(), asRecord(), asString(), clampColorIndex(), EVENT, isForNote(), NotePresenceOptions, parseViewer() (+9 more)

### Community 256 - "attachment-storage-key.ts"
Cohesion: 0.11
Nodes (23): FULL, fullRecord(), imageVariant(), MEDIUM, ORIGINAL, PREVIEW, THUMBNAIL, assertUuid() (+15 more)

### Community 257 - "Environment contracts"
Cohesion: 0.10
Nodes (16): Authentication, freshness, rate limiting, and errors, Canonical contracts, Centralized authorization, Known boundaries, Permissions matrix, Tenant scoping and loading, Transport and job invocation, Authentication and email queue values (+8 more)

### Community 258 - "ADR 0001: pnpm and Turborepo monorepo boundaries"
Cohesion: 0.29
Nodes (7): ADR 0001: pnpm and Turborepo monorepo boundaries, Alternatives considered, Consequences, Context, Decision, Migration and rollback, Tenancy and data flow

### Community 259 - "ADR 0004: Yjs collaborative document authority over Socket.io transport"
Cohesion: 0.22
Nodes (8): ADR 0004: Yjs collaborative document authority over Socket.io transport, Alternatives considered, Consequences, Context, Decision, Migration and rollback, Part 58 amendment — asynchronous projection, cadence, and presence identity, Safe defaults and unresolved details

### Community 260 - "ApiKeys.tsx"
Cohesion: 0.14
Nodes (13): apiKey, create, list, revoke, ApiKeys(), formatDate(), SCOPES, apiKeySearch() (+5 more)

### Community 261 - "overrides"
Cohesion: 0.25
Nodes (8): body-parser@<1.20.6, esbuild@<0.25.0, file-type@<21.3.2, @nestjs/platform-express@10.4.22>multer, next@16.2.11>postcss, next@16.2.11>sharp, qs@<6.15.2, overrides

### Community 262 - "note-yjs-document.ts"
Cohesion: 0.09
Nodes (27): document, harness(), Mocked, noteRow, records, snapshot, state, childrenToJson() (+19 more)

### Community 263 - "Phase 1 — Product Decisions and Repository Foundation"
Cohesion: 0.25
Nodes (8): Part 1 — Record architecture decisions and resolve specification gaps, Part 2 — Initialize the pnpm/Turborepo monorepo, Part 3 — Establish formatting, linting, and commit quality gates, Part 4 — Scaffold the Next.js web application, Part 5 — Scaffold the NestJS API application, Part 6 — Create shared types and validators, Part 7 — Add continuous integration, Phase 1 — Product Decisions and Repository Foundation

### Community 264 - "Phase 5 — Core Workspace, Project, and Note Management"
Cohesion: 0.25
Nodes (8): Part 26 — Implement workspace lifecycle APIs, Part 27 — Build workspace screens, Part 28 — Implement membership and invitation flows, Part 29 — Implement project CRUD APIs, Part 30 — Build project list and detail screens, Part 31 — Implement core note APIs, Part 32 — Build note browsing and hierarchy UI, Phase 5 — Core Workspace, Project, and Note Management

### Community 265 - "Phase 6 — Rich Editor and Paper Experience"
Cohesion: 0.25
Nodes (8): Part 33 — Establish the TipTap document contract, Part 34 — Build the basic editor and toolbar, Part 35 — Add tables, checklists, markdown shortcuts, and block behavior, Part 36 — Build slash commands and mentions, Part 37 — Implement the A4/Letter page container, Part 38 — Add page breaks, focus mode, and print styling, Part 39 — Implement reliable save behavior, Phase 6 — Rich Editor and Paper Experience

### Community 266 - "Build and Diagnose Notted Playwright Integration Tests"
Cohesion: 0.29
Nodes (6): Author Reliable Journeys, Build and Diagnose Notted Playwright Integration Tests, Completion Evidence, Diagnose Failures, Stable Runner Setup, Test the Real Boundary

### Community 267 - "nest-cli.json"
Cohesion: 0.29
Nodes (6): collection, compilerOptions, deleteOutDir, tsConfigPath, $schema, sourceRoot

### Community 268 - "note-version-checkpoint.policy.ts"
Cohesion: 0.27
Nodes (9): CHECKPOINT_MIN_INTERVAL_MS, CollaborativeCheckpointDecision, CollaborativeCheckpointInput, CollaborativeCheckpointReason, decideCollaborativeCheckpoint(), isCheckpointEligibleMutation(), NON_COLLABORATIVE_CHECKPOINT_MUTATIONS, NOW (+1 more)

### Community 269 - "app/layout.tsx"
Cohesion: 0.38
Nodes (3): metadata, viewport, ToasterProvider()

### Community 270 - "agents/backend-platform-engineer.md"
Cohesion: 0.29
Nodes (6): Authority chain, Completion payload, Delegation (Synchronous Delegation Protocol, Claude Code mechanism), Operating rules, Skill to load, Verification

### Community 271 - "agents/frontend-editor-engineer.md"
Cohesion: 0.29
Nodes (6): Authority chain, Completion payload, Delegation (Synchronous Delegation Protocol, Claude Code mechanism), Operating rules, Skills to load, Verification

### Community 272 - "agents/lead-part-engineer.md"
Cohesion: 0.29
Nodes (6): Authority chain, Completion payload, Delegation, Operating rules, Skill to load, Synchronous Delegation Protocol (Claude Code mechanism)

### Community 273 - "agents/quality-reviewer.md"
Cohesion: 0.29
Nodes (6): Authority chain, Completion payload, Delegation (Synchronous Delegation Protocol, Claude Code mechanism), Operating rules, Read-only contract, Skill to load

### Community 274 - "Build and Diagnose Notted Playwright Integration Tests"
Cohesion: 0.29
Nodes (6): Author Reliable Journeys, Build and Diagnose Notted Playwright Integration Tests, Completion Evidence, Diagnose Failures, Stable Runner Setup, Test the Real Boundary

### Community 275 - "Database migration policy"
Cohesion: 0.29
Nodes (7): Application, Database migration policy, Generate and review, Immutability, Migration 0007 note-version preflight, Required tests, Rollback

### Community 276 - "ADR 0002: Separate first-party tRPC and public REST transports"
Cohesion: 0.29
Nodes (6): ADR 0002: Separate first-party tRPC and public REST transports, Alternatives considered, Consequences, Context, Decision, Migration and rollback impact

### Community 277 - "ADR 0005: Store private binaries in MinIO with PostgreSQL metadata"
Cohesion: 0.29
Nodes (6): ADR 0005: Store private binaries in MinIO with PostgreSQL metadata, Alternatives considered, Consequences, Context, Decision, Migration and rollback impact

### Community 278 - "ADR 0010: Persist Better Auth sessions and encrypted authentication-email intent"
Cohesion: 0.29
Nodes (6): ADR 0010: Persist Better Auth sessions and encrypted authentication-email intent, Alternatives considered, Consequences, Context, Decision, Migration and rollback impact

### Community 279 - "ADR 0011: Explicit durable project restriction state"
Cohesion: 0.29
Nodes (6): ADR 0011: Explicit durable project restriction state, Alternatives considered, Consequences, Context, Decision, Migration and rollback

### Community 280 - "Phase 7 — Images and Attachments"
Cohesion: 0.29
Nodes (7): Part 40 — Build secure object-storage services, Part 41 — Implement image ingestion and processing, Part 42 — Integrate editor image insertion, Part 43 — Add image manipulation UI, Part 44 — Implement generic attachment flows, Part 45 — Add storage quotas and cleanup, Phase 7 — Images and Attachments

### Community 281 - "Phase 10 — Versioning, Comments, and Real-Time Collaboration"
Cohesion: 0.29
Nodes (7): Part 55 — Implement version snapshot and retention logic, Part 56 — Build version history, diff, and restore UI, Part 57 — Build authenticated Socket.io infrastructure, Part 58 — Integrate Yjs collaborative editing, Part 59 — Add presence and collaboration UI, Part 60 — Implement inline comments and mentions, Phase 10 — Versioning, Comments, and Real-Time Collaboration

### Community 282 - "Phase 11 — Export, Email, API Keys, and Webhooks"
Cohesion: 0.29
Nodes (7): Part 61 — Build the email subsystem, Part 62 — Implement export job lifecycle, Part 63 — Implement PDF and HTML export, Part 64 — Implement Markdown, TXT, DOCX, and ZIP export, Part 65 — Implement public REST API and API key management, Part 66 — Implement webhooks and delivery logs, Phase 11 — Export, Email, API Keys, and Webhooks

### Community 283 - "Notted Development Plan"
Cohesion: 0.09
Nodes (22): Completion Records, Milestone Checkpoints, Notted Development Plan, Part 67 — Build provider-neutral AI configuration and governance, Part 68 — Implement summarize, continue writing, and tone rewrite, Part 69 — Implement meeting extraction and auto-tagging, Part 70 — Implement grammar and style assistance, Part 75 — Complete the automated test pyramid (+14 more)

### Community 284 - "tsconfig.json"
Cohesion: 0.29
Nodes (6): compilerOptions, extends, files, include, ./tsconfig.base.json, references

### Community 285 - "collaboration.spec.ts"
Cohesion: 0.15
Nodes (16): Account, apiGet(), apiPatch(), apiPost(), characterCounts(), collabStatus(), containsEveryCharacter(), editorBody() (+8 more)

### Community 286 - "Part 63 — PDF and HTML export"
Cohesion: 0.22
Nodes (9): Corrections made during implementation, Fixed after review, Implemented Work, Objective, Open risks and follow-ups, Part 63 — PDF and HTML export, Status, The central problem: the app→app boundary (+1 more)

### Community 287 - "graphify reference: query, path, explain"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 288 - "Build Notted Frontend and Editor Features"
Cohesion: 0.33
Nodes (5): Behavior, Boundaries, Build Notted Frontend and Editor Features, Completion payload, Verify

### Community 289 - "graphify reference: query, path, explain"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 290 - "src/format-bytes.ts"
Cohesion: 0.47
Nodes (5): BINARY_UNITS, DECIMAL_FORMAT, exactByteLabel(), formatBinaryBytes(), INTEGER_FORMAT

### Community 291 - "Phase 4 — Authentication, Authorization, and Application Shell"
Cohesion: 0.33
Nodes (6): Part 21 — Integrate Better Auth on the backend, Part 22 — Build authentication screens and route protection, Part 23 — Add OAuth, two-factor authentication, passkeys, and session controls, Part 24 — Implement centralized authorization, Part 25 — Build the dashboard shell, Phase 4 — Authentication, Authorization, and Application Shell

### Community 292 - "Phase 9 — Search and Indexing"
Cohesion: 0.33
Nodes (6): Part 50 — Establish BullMQ queues and workers, Part 51 — Build the Meilisearch indexing pipeline, Part 52 — Implement full-text search APIs and UI, Part 53 — Add embeddings and semantic search, Part 54 — Implement hybrid ranking, Phase 9 — Search and Indexing

### Community 293 - "inputs"
Cohesion: 0.33
Nodes (6): .env.example, .env.local, .env.production, next.config.{js,ts,mjs,cjs}, public/**/*, inputs

### Community 294 - "Build Notted Frontend and Editor Features"
Cohesion: 0.40
Nodes (4): Behavior, Boundaries, Build Notted Frontend and Editor Features, Verify

### Community 295 - "Deliver a Notted Plan Part"
Cohesion: 0.40
Nodes (4): Deliver a Notted Plan Part, Establish context, Execute, Verify and hand off

### Community 296 - "api/package.json"
Cohesion: 0.40
Nodes (4): description, name, private, version

### Community 297 - "webhook-url-guard.ts"
Cohesion: 0.15
Nodes (19): BLOCKED_ADDRESSES, BLOCKED_HOST_SUFFIXES, BLOCKED_HOSTS, defaultLookup(), inspectWebhookUrl(), isBlockedAddress(), LOOPBACK_ADDRESSES, normalizeAddress() (+11 more)

### Community 298 - "Part 52 — Authorized full-text search"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 299 - "pdf-export.service.test.ts"
Cohesion: 0.06
Nodes (25): EXPORT_CONFIG, ExportConfig, ChromiumUnavailableError, LAUNCH_ARGS, config(), fakeBrowser(), fakeContext(), logger() (+17 more)

### Community 300 - "shell.schema.ts"
Cohesion: 0.11
Nodes (19): NotificationEmailPreferenceInput, notificationEmailPreferenceSchema, notificationKindSchema, NotificationListQueryInput, notificationListQuerySchema, notificationPageSchema, notificationReadResultSchema, NotificationReadStateInput (+11 more)

### Community 301 - "Build Notted Backend and Data Features"
Cohesion: 0.40
Nodes (4): Architecture and safety, Build Notted Backend and Data Features, Completion payload, Verify

### Community 302 - "Assure Notted Quality and Operations"
Cohesion: 0.40
Nodes (4): Assure Notted Quality and Operations, Completion payload, Evidence and reporting, Review order

### Community 303 - "Part 53 — Embeddings and semantic search"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 304 - "Part 54 — Hybrid ranking"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 305 - "[projectId]/page.tsx"
Cohesion: 0.09
Nodes (34): ProjectsPage(), { notFound }, query, workspace, formatActivity(), ProjectDetailPage(), CreateProjectModal(), { router } (+26 more)

### Community 306 - "advanced-auth.spec.ts"
Cohesion: 0.29
Nodes (3): decodeBase32(), registerAndVerify(), totpFromUri()

### Community 307 - "Phase 16 — Release Preparation and Incremental Delivery"
Cohesion: 0.40
Nodes (5): Part 85 — Define and validate the MVP release slice, Part 86 — Run staging acceptance and release rehearsal, Part 87 — Release the MVP and monitor it, Part 88 — Deliver post-MVP capabilities in controlled increments, Phase 16 — Release Preparation and Incremental Delivery

### Community 308 - "Build Notted Backend and Data Features"
Cohesion: 0.50
Nodes (3): Architecture and safety, Build Notted Backend and Data Features, Verify

### Community 309 - "Assure Notted Quality and Operations"
Cohesion: 0.50
Nodes (3): Assure Notted Quality and Operations, Evidence and reporting, Review order

### Community 312 - "graphify reference: add a URL and watch a folder"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 313 - "graphify reference: commit hook and native CLAUDE.md integration"
Cohesion: 0.50
Nodes (3): For git commit hook, For native CLAUDE.md integration, graphify reference: commit hook and native CLAUDE.md integration

### Community 314 - "graphify reference: incremental update and cluster-only"
Cohesion: 0.50
Nodes (3): For --cluster-only, For --update (incremental re-extraction), graphify reference: incremental update and cluster-only

### Community 315 - "opencode.json"
Cohesion: 0.50
Nodes (3): instructions, $schema, AGENTS.md

### Community 316 - ".opencode/opencode.json"
Cohesion: 0.50
Nodes (3): plugin, $schema, .opencode/plugins/graphify.js

### Community 317 - "graphify reference: add a URL and watch a folder"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 318 - "graphify reference: commit hook and native CLAUDE.md integration"
Cohesion: 0.50
Nodes (3): For git commit hook, For native CLAUDE.md integration, graphify reference: commit hook and native CLAUDE.md integration

### Community 319 - "graphify reference: incremental update and cluster-only"
Cohesion: 0.50
Nodes (3): For --cluster-only, For --update (incremental re-extraction), graphify reference: incremental update and cluster-only

### Community 326 - "Part 61 — Email subsystem"
Cohesion: 0.25
Nodes (8): Deviations and deferrals, Fixed after review, Implemented Work, Objective, Open risks and follow-ups, Part 61 — Email subsystem, Status, Verification

### Community 331 - "Part 56 — Version history, diff, and restore"
Cohesion: 0.22
Nodes (8): API, Diff and preview, Part 56 — Version history, diff, and restore, Result, Risks and follow-up, Search and collaboration boundaries, Status, Tests authored and verification

### Community 332 - "StorageBucket"
Cohesion: 0.20
Nodes (3): MemoryObjectStore, StorageBucket, PrefixedObjectStore

### Community 333 - "storage-maintenance.integration.test.ts"
Cohesion: 0.03
Nodes (84): build(), fakeDatabase(), FakeRow, fileInput(), PDF, PNG, principal(), quotaService() (+76 more)

### Community 334 - "Part 64 — Markdown, TXT, DOCX and ZIP export, and the export UI"
Cohesion: 0.25
Nodes (8): Dependency note, Deviations, Fixed after review, Implemented Work, Objective, Open risks and follow-ups, Part 64 — Markdown, TXT, DOCX and ZIP export, and the export UI, Status

### Community 335 - "multipart-upload.parser.ts"
Cohesion: 0.14
Nodes (12): badRequest(), headerValue(), MULTIPART_OVERHEAD_BYTES, MULTIPART_TIMEOUT_MS, MultipartRequest, ParsedSingleFileUpload, parseSingleFileUpload(), ParseSingleFileUploadOptions (+4 more)

### Community 336 - "PresenceBar.tsx"
Cohesion: 0.27
Nodes (12): COLLAB_STATUS_TEST_ID, CollaborationStatus(), CollaborationStatusProps, MESSAGE, initialsOf(), PresenceBar(), PresenceBarProps, RECONNECTABLE (+4 more)

### Community 337 - "note-collaboration-provider.ts"
Cohesion: 0.13
Nodes (19): ACK_ERRORS, AckError, AckOutcome, asBinary(), asInteger(), asRecord(), EVENT, isForNote() (+11 more)

### Community 338 - "realtime.gateway.test.ts"
Cohesion: 0.16
Nodes (14): awarenessFrame(), config, Handler, handlerFor(), harness(), invoke(), joinNote(), otherSelector (+6 more)

### Community 339 - "Part 55 — Version snapshots and retention"
Cohesion: 0.25
Nodes (7): Files and architecture, Limits and follow-up, Migration, Part 55 — Version snapshots and retention, Result, Status, Tests authored and verification

### Community 340 - "Part 57 — Build authenticated Socket.io infrastructure"
Cohesion: 0.24
Nodes (6): Implemented, Migration, Operations and integration contract, Part 57 — Build authenticated Socket.io infrastructure, Risks, Verification

### Community 341 - "project.ts"
Cohesion: 0.17
Nodes (11): PROJECT_API_PATHS, ProjectAccessRole, ProjectCreateResult, ProjectListQuery, ProjectMemberAccessSource, ProjectPage, ProjectSortField, ProjectStatus (+3 more)

### Community 342 - "pnpm"
Cohesion: 0.33
Nodes (6): ignoreCves, pnpm, auditConfig, onlyBuiltDependencies, esbuild, GHSA-36xv-jgw5-4q75

### Community 343 - "StructuredLogger"
Cohesion: 0.02
Nodes (106): AuthEmailJobContext, appConfig, Inject, BetterAuthSetupDependencies, LogMetadata, StructuredLogger, Inject, Injectable (+98 more)

### Community 345 - "common.ts"
Cohesion: 0.17
Nodes (13): ApiError, ApiErrorCode, ApiFailure, ApiSuccess, ValidationErrorDetails, ValidationIssue, JsonPrimitive, JsonValue (+5 more)

### Community 349 - "realtime-socket.adapter.ts"
Cohesion: 0.25
Nodes (3): RealtimeConfig, RealtimeSocketAdapter, config

### Community 351 - "export.service.ts"
Cohesion: 0.05
Nodes (40): ExportJobProducer, Injectable, CANCELLABLE_STATUSES, CreateExportServiceInput, EXPORT_FAILURE_MESSAGES, ExportClaim, ExportDownloadContent, ExportFailureCode (+32 more)

### Community 352 - "MemoryObjectStore"
Cohesion: 0.20
Nodes (4): key(), expectObjectsAbsent(), expectObjectsPresent(), MemoryObjectStore

### Community 355 - "Phase 8 — Tasks, Tags, Templates, and Advanced Organization"
Cohesion: 0.40
Nodes (5): Part 46 — Implement tags and templates, Part 47 — Implement standalone task CRUD and list view, Part 48 — Add board, calendar, and progress views, Part 49 — Add project board and timeline views, Phase 8 — Tasks, Tags, Templates, and Advanced Organization

### Community 356 - "authorization-http.guard.ts"
Cohesion: 0.10
Nodes (21): authorizationDenialToHttpException(), AUTHORIZED_OPERATION, AuthorizedRequest, getAuthorizedOperation(), setAuthorizedOperation(), AuthorizationHttpGuard, ADR-0003, Injectable (+13 more)

### Community 357 - "folder-controls.test.tsx"
Cohesion: 0.33
Nodes (4): child, grandchild, mocks, root

### Community 358 - "NoteBoard.tsx"
Cohesion: 0.14
Nodes (9): item(), NoteViewProps, buildNoteColumns(), NoteBoard(), NoteBoardColumn, noteColumnIdOf(), NoteList(), NoteMoveDestination (+1 more)

### Community 359 - "ApiKeysController"
Cohesion: 0.17
Nodes (11): ApiKeysController, routeUuid(), Body, Controller, Delete, Get, HttpCode, Post (+3 more)

### Community 362 - "user-color.ts"
Cohesion: 0.73
Nodes (4): PRESENCE_COLOR_COUNT, presenceColorForUser(), presenceColorIndex(), presenceColorVar()

### Community 363 - "storage-report.ts"
Cohesion: 0.33
Nodes (8): assertReportOnlyArguments(), main(), renderReport(), renderSweepDetails(), renderSweepTable(), REPORT_COLUMNS, ReportColumn, StorageReportError

### Community 364 - "TagsService"
Cohesion: 0.26
Nodes (3): TagsService, Injectable, buildTagSubrouter()

### Community 365 - "note-collaboration-provider.test.ts"
Cohesion: 0.22
Nodes (6): AckCallback, completeHandshake(), EmittedFrame, flush(), Harness, SocketListener

### Community 366 - "bull-board.service.ts"
Cohesion: 0.16
Nodes (6): BullBoardService, Injectable, REDACTED, RedactedBullMqAdapter, redactedJob(), safeJobJson()

### Community 370 - "api-keys.e2e.test.ts"
Cohesion: 0.12
Nodes (5): ENVIRONMENT_KEYS, EnvironmentSnapshot, KEY_NAME_PREFIXES, MIGRATIONS_FOLDER, SeededKey

### Community 371 - "ai-provider-rate-limiter.service.ts"
Cohesion: 0.16
Nodes (9): AiProviderQueueLimitConfig, REDIS_CLIENT, AiProviderRateLimiterService, AiRateLimitedProvider, AiRateLimitResult, ResolvedProviderLimit, config, Inject (+1 more)

### Community 391 - "Resources"
Cohesion: 0.12
Nodes (17): API keys, Attachments, Comments, Exports, Folders, Members and invitations, Note shares, Notes (+9 more)

### Community 400 - "api-keys.service.test.ts"
Cohesion: 0.13
Nodes (11): Awaitable, createdAt, createInput, dialect, HarnessOptions, listInput, principal, rows() (+3 more)

### Community 401 - "NoteTimeline.tsx"
Cohesion: 0.12
Nodes (22): broken, designDoc, kickoff, mocks, note(), onePage, task(), DATE_FORMAT (+14 more)

### Community 425 - "webhook-sender.ts"
Cohesion: 0.19
Nodes (12): ANSI_SEQUENCE, buildSnippet(), classifySocketError(), sendWebhook(), RELAXED, servers, TestServer, TLS_ERROR_CODES (+4 more)

### Community 430 - "upload-request.ts"
Cohesion: 0.20
Nodes (13): NoteRequestResult, conflictKind(), failureFor(), IMAGE_UPLOAD_FILE_FIELD, IMAGE_UPLOAD_TIMEOUT_MS, parseBody(), retryAfterMs(), attachmentPayload() (+5 more)

### Community 431 - "Public REST API"
Cohesion: 0.12
Nodes (16): 404 instead of 403 across workspaces, API keys, Authentication, Base URL and versioning, Effective permission: scope ∩ creator's live role, Envelope, Errors, Idempotency (+8 more)

### Community 433 - "Part 65 — Implement public REST API and API key management"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 435 - "Part 66 — Implement webhooks and delivery logs"
Cohesion: 0.14
Nodes (13): API, Configuration, and Operational Changes, Database and Data Changes, Files and Components, Handoff Notes, Implemented Work, Important Decisions, Known Limitations and Follow-up Work, Objective (+5 more)

### Community 438 - "SearchResultRepository"
Cohesion: 0.17
Nodes (7): Inject, Optional, SearchResultRepository, Injectable, uniq(), Inject, Optional

### Community 440 - "FakeObjectStore"
Cohesion: 0.17
Nodes (3): ago(), busyFixture(), FakeObjectStore

### Community 448 - ".runMaintenance"
Cohesion: 0.22
Nodes (8): StorageController, Body, Controller, Get, HttpCode, Post, Req, workspaceIdFromRoute()

### Community 452 - "task-calendar.test.tsx"
Cohesion: 0.18
Nodes (10): done, mocks, openCalendar(), overdue, page, startOfToday, task(), today (+2 more)

### Community 455 - "workspaceId"
Cohesion: 0.04
Nodes (41): whereValuesFixture(), attachmentRow(), commentRow(), versionRow(), principal, service(), moveInput(), scopesToWorkspace() (+33 more)

### Community 457 - "tasks/view-preference.ts"
Cohesion: 0.35
Nodes (7): isPreference(), PreferenceStorage, readTaskViewPreference(), TaskViewPreference, taskViewPreferenceKey(), VALUES, writeTaskViewPreference()

### Community 459 - "web/package.json"
Cohesion: 0.40
Nodes (4): description, name, private, version

### Community 461 - "ShellService"
Cohesion: 0.19
Nodes (8): ShellController, Controller, Get, Query, Req, UseGuards, ShellService, Injectable

### Community 462 - "embedding-provider.ts"
Cohesion: 0.10
Nodes (15): AiConfig, EMBEDDING_DIMENSIONS, EMBEDDING_PROVIDER, EmbeddingAvailability, EmbeddingProvider, EmbeddingProviderError, EmbeddingResult, isUsableEmbeddingVector() (+7 more)

### Community 463 - "src/auth.ts"
Cohesion: 0.17
Nodes (12): AUTH_API_PATHS, AuthCapabilities, AuthEmailAccepted, AuthenticationAssurance, AuthenticationMethod, AuthPasskeySummary, AuthSecurityOverview, AuthSessionSummary (+4 more)

### Community 468 - "decodeAwarenessClientIds"
Cohesion: 0.60
Nodes (3): decodeAwarenessClientIds(), awarenessUpdate(), varUint()

### Community 476 - "note-board.test.tsx"
Cohesion: 0.22
Nodes (9): first, mocks, note(), openBoard(), page, query, review, second (+1 more)

### Community 477 - "presence-bar.test.tsx"
Cohesion: 0.24
Nodes (6): memberPage(), mocks, PEOPLE, userIdAt(), view(), PresenceRoster

### Community 481 - "HealthController"
Cohesion: 0.24
Nodes (6): RateLimitExempt(), HealthController, Controller, Get, Inject, Res

### Community 482 - "queue-worker-processor.service.test.ts"
Cohesion: 0.22
Nodes (8): bullJobEnvelopeSchema, JOB_REGISTRY, registeredJobDefinition(), config(), invocation, payload, runtimeRow, setup()

### Community 483 - "OutboxDispatcherService"
Cohesion: 0.20
Nodes (3): OutboxDispatcherService, Injectable, DeadLetterRecord

### Community 484 - "tag.ts"
Cohesion: 0.28
Nodes (8): TagId, TagCreateResult, TagDeleteResult, TagListQuery, TagPage, TagSortField, TagSummary, TagUpdateResult

### Community 485 - "Webhooks"
Cohesion: 0.18
Nodes (11): Delivery logs, Destination restrictions, Endpoint lifecycle, Event catalog, Headers, Payload, Retry and backoff, Secrets (+3 more)

### Community 488 - "tag-manager.test.tsx"
Cohesion: 0.22
Nodes (7): created, design, mocks, page, research, tag(), TagManager()

### Community 489 - ".deliver"
Cohesion: 0.31
Nodes (4): authorizationTarget(), classify(), Injectable, WebhookDeliveryWorkerService

### Community 490 - "ShareModal.tsx"
Cohesion: 0.33
Nodes (6): mocks, mutationPermission(), ShareModal(), fetchWorkspaceMemberDirectory(), requestAllWorkspaceMembers(), WORKSPACE_MEMBER_DIRECTORY_LIMIT

### Community 491 - "export-object-key.ts"
Cohesion: 0.47
Nodes (4): exportDownloadFilename(), exportObjectKey(), ADR-0005, RFC-5987

### Community 492 - "api-keys/index.ts"
Cohesion: 0.33
Nodes (7): GeneratedApiKeySecret, API_KEY_AUDIT_ACTIONS, API_KEY_AUDIT_ENTITY_TYPE, API_KEY_LAST_USED_THROTTLE_MS, API_KEY_PREFIX, ApiKeysModule, Module

### Community 493 - "Phase 13 — Enterprise Controls and Customization"
Cohesion: 0.40
Nodes (5): Part 71 — Add audit logging and administrative views, Part 72 — Implement branding and customization, Part 73 — Implement custom-domain support, Part 74 — Harden security and abuse controls, Phase 13 — Enterprise Controls and Customization

### Community 494 - "QueueAdminRemediationService"
Cohesion: 0.31
Nodes (3): QueueAdminRemediationService, rowsOf(), Injectable

### Community 495 - "api-key-secret.ts"
Cohesion: 0.54
Nodes (6): formatScopes(), generateApiKeySecret(), hashApiKey(), parseScopes(), seedKey(), seedKey()

### Community 497 - "api-keys.service.ts"
Cohesion: 0.43
Nodes (6): ApiKeyRow, CreateApiKeyServiceInput, ListApiKeysServiceInput, RevokeApiKeyServiceInput, ScopedInput, ADR-0009

### Community 500 - "presence-color.ts"
Cohesion: 0.60
Nodes (3): PRESENCE_COLOR_COUNT, presenceColorIndex(), FIXTURES

## Knowledge Gaps
- **3081 isolated node(s):** `$schema`, `.opencode/plugins/graphify.js`, `semi`, `singleQuote`, `trailingComma` (+3076 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **100 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `workspaceId()` connect `workspaceId` to `NoteIndexRepository`, `task-board.test.tsx`, `AuthService`, `task-calendar.test.tsx`, `NoteBoard.tsx`, `TaskStatusesController`, `request-json.ts`, `tag-manager.test.tsx`, `TaskListView.tsx`, `storage-maintenance.integration.test.ts`, `createTenantContext`, `upload-request.ts`, `server-workspaces.ts`, `NoteTimeline.tsx`, `note-export-source.service.ts`, `note-board.test.tsx`, `image-uploads.ts`, `export.service.ts`?**
  _High betweenness centrality (0.122) - this node is a cross-community bridge._
- **Why does `setup()` connect `image-uploads.ts` to `workspaceId`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Why does `bearerSecret()` connect `api-key-auth.service.ts` to `common.schema.ts`?**
  _High betweenness centrality (0.042) - this node is a cross-community bridge._
- **What connects `$schema`, `.opencode/plugins/graphify.js`, `semi` to the rest of the system?**
  _3081 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `shared-validators/src/index.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.03238919485969053 - nodes in this community are weakly interconnected._
- **Should `schema/index.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.021574973031283712 - nodes in this community are weakly interconnected._
- **Should `document.schema.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.04112554112554113 - nodes in this community are weakly interconnected._