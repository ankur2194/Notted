...
- **`pnpm test` measures no coverage at all.** It is `vitest run` with no `--coverage`, so none of the thresholds — the global 70s, nor the `src/tenant/**` 95, `src/authorization/**` 85, `src/auth/**` 75, `src/common/idempotency/**` 95 floors in `apps/api/vitest.config.ts` — are evaluated by it. A green `pnpm test` says the tests that ran passed; it says nothing about what did not run. The real gate is the container-scoped `test:ci` invocation below.
- `pnpm test:ci` (coverage thresholds) needs `DATABASE_URL` exported and the dev stack up (`pnpm infra:up:ports`). Without it 17 API suites skip silently and branch coverage lands around 62%, which reads as a real regression but is not one. `turbo.json` documents the passthrough. `test/integration-gates.test.ts` now fires on any run that sets `DATABASE_URL` — a developer laptop included — and fails loudly if the rest of the stack is not configured; it used to additionally require `CI`, which this repository does not set anywhere, so it had never executed.
