...
import { applyBulk, deletionScope, failureMessage, optimisticTask } from "@/lib/tasks/optimistic";
import { canDeleteTasks, canEditTask, canUnassign } from "@/lib/tasks/permissions";
import {
  bulkUpdateTasks,
  createTask,
  deleteTask,
...
export function TaskListView({
   ...
   */
  const editableTask = (task: TaskSummary): boolean => canEditTask(canEdit, viewer, task);
  const deletable = canDeleteTasks(canEdit, viewer);
  const unassignable = canUnassign(viewer);
  ...
  function renderTask(task: TaskSummary, movement: TaskMovement): ReactNode {
    return (
      <TaskRow
        ...
        disabled={!editableTask(task) || operationPending}
        canDelete={deletable}
        canUnassign={unassignable}
