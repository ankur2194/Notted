...
- **The rule is now enforced, not merely written down.** `apps/web/playwright.config.ts` refuses to start when `PLAYWRIGHT_APP_URL` is unset outside CI, so a bare `playwright test` fails closed with an instruction instead of silently targeting `localhost:3000`. The root `pnpm test:e2e` was rewired to the disposable path.
