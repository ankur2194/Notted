...
- **State:** Verified and closed — gates run and passing; the licence sign-off is obtained
- **Completed on:** 2026-08-07 (the `libheif-js` LGPL-3.0 decision was taken the same day: **keep**, obligations discharged through attribution in [`THIRD-PARTY-NOTICES.md`](../../THIRD-PARTY-NOTICES.md))
...
5. **HEIC needs a JavaScript decoder because Sharp genuinely cannot decode it here.** The installed `sharp@0.35.0` bundles prebuilt libvips 8.18.3, which reports `format.heif.input.fileSuffix === [".avif"]`; HEIC would require a globally installed libvips built with libheif/libde265/x265, which the container does not ship. `Notted.md` requires "HEIC (convert to JPEG)", so `heic-convert@2.1.0` decodes to JPEG first and the JPEG then travels the ordinary pipeline. **Licence — signed off 2026-08-07, decision: keep.** The chain reaches `libheif-js@1.19.8`, which is **LGPL-3.0**. LGPL obligations trigger on conveying rather than on use (it is not AGPL), and where Notted does convey, the package is separately replaceable rather than statically linked into Notted, so the relinking obligation is satisfied structurally. That remained a judgement rather than a fact, which is why it was escalated; the project owner confirmed it and elected to discharge the obligations through attribution. Recorded in ADR 0008 and, authoritatively, in [`THIRD-PARTY-NOTICES.md`](../../THIRD-PARTY-NOTICES.md).
...
| Real HEIC round trip | Not run, and not runnable automatically | Unchanged: requires `NOTTED_TEST_HEIC_FIXTURE` pointing at a real `.heic`. Sharp cannot generate one (no HEVC encoder) and a sample is patent-encumbered. The test skips with that exact message. **Manual verification only.** |
| `pnpm audit --prod --audit-level=high` | Pass | 3 moderate, all pre-existing. |
| `libheif-js` LGPL-3.0 sign-off | **Obtained 2026-08-07 — decision: keep** | A human decision, not a command; taken by the project owner. Obligations discharged through attribution: see [`THIRD-PARTY-NOTICES.md`](../../THIRD-PARTY-NOTICES.md) and the HEIC row in ADR 0008. Was correctly escalated rather than decided or silenced by an agent. Correction to the framing: it was **not** the only copyleft dependency in the tree — the sign-off session's audit also found `@img/sharp-libvips-linux-x64@1.3.0` (LGPL-3.0-or-later), reached via `sharp`. The owner was shown that finding and **extended the same sign-off to it the same day**. |
...
- **`libheif-js` LGPL-3.0 — resolved 2026-08-07: keep.** The project owner signed off; the obligations are discharged through attribution in [`THIRD-PARTY-NOTICES.md`](../../THIRD-PARTY-NOTICES.md), which is now the authoritative record, with the summary in ADR 0008's HEIC row. The escape hatch stays available and is unchanged: deleting `heic-decoder.ts` and the two package pins removes HEIC support cleanly, because `supports()` already returns 415 up front and no other file references the decoder. **Two things carry forward.** First, the standing obligations are ongoing rules, not closed chores: never modify the package, ship the notice with anything distributed, and keep the package separately replaceable — so adopting a bundler for `apps/api` would invalidate the analysis and require re-doing it. Second, this record's earlier claim that `libheif-js` was the tree's only copyleft dependency was **wrong**: `@img/sharp-libvips-linux-x64@1.3.0` is LGPL-3.0-or-later and arrives with `sharp`. The owner extended the sign-off to cover it on 2026-08-07 under the same analysis. That correction also retires the "drop HEIC for a copyleft-free tree" escape hatch as a *licence* argument — that package predates Part 41, entering through the reviewed `next > sharp` override, so a copyleft-free tree would mean dropping `sharp` and the image pipeline entirely. Removing `heic-decoder.ts` remains a clean one-file change, but it is now purely a feature decision.
...
| 2026-08-07 | Licence sign-off pass | Recorded the project owner's decision to **keep** `libheif-js@1.19.8` and discharge the LGPL-3.0 obligations through attribution. Added `THIRD-PARTY-NOTICES.md`, updated ADR 0008 and the `heic-decoder.ts` header comment (comment text only — no executable code changed), and closed the outstanding handoff item. Built and verified a repeatable copyleft re-audit command, which corrected this record's premise by surfacing a second LGPL component, `@img/sharp-libvips-linux-x64@1.3.0`, reached via `sharp`. The owner was shown that finding and extended the sign-off to cover it under the same analysis, so both LGPL components in the tree are now decided. |
