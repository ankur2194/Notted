...
- **Related records:** Parts 07, 33, 34, 35, and 36

## Objective

`pnpm test:ci` failed. Make it pass — locally and in CI — with every workspace genuinely at
...
`.github/workflows/ci.yml` runs `pnpm test:ci`, so CI was red for the same reasons.
...
- **api** — 16 test files gate themselves on `DATABASE_URL` via `describe.skipIf`. Without a
  ...
  CI had no service containers, so those tests had never run there.
...
- Added a `services:` block to the `quality` job in `.github/workflows/ci.yml`: a
  digest-pinned `pgvector/pgvector:0.8.5-pg16` (**not** stock `postgres` — migration
  `0000_enable_extensions.sql` enables the `vector` extension), `pg_isready` health options,
  and throwaway credentials. `DATABASE_URL` is set on the test step only, so `pnpm build`
  and `pnpm db:check` keep their own configuration.
- Declared `"env": ["DATABASE_URL"]` on the `test` and `test:ci` tasks in `turbo.json`.
  **Without this the workflow change does nothing**: Turbo filters the environment strictly,
...
| Path | Change |
|---|---|
| `.github/workflows/ci.yml` | pgvector service container; `DATABASE_URL` scoped to the test step |
...
No schema, migration, or seed change. CI now provisions an ephemeral database per run.
...
another user's session, and `Error` messages never being logged verbatim. The CI database
credentials are throwaway values for a runner-local, ephemeral container.
...
| `pnpm test:ci` with `pnpm infra:down` | **Fail (expected)** | `apps/api` 55.78% statements; the gated suites skip. This is the condition the CI service container removes |
| api suite vs. an empty database | Pass | 635 passed, 3 skipped — the exact CI condition, verified with a throwaway database created without the dev init script |
...
- **The CI workflow change itself is unverified.** It cannot be exercised from a local
  checkout. The evidence for it is indirect but specific: the API suite was run against a
  throwaway database created without the development init script, which is what a GitHub
  service container provides, and it passed with the numbers above. The first CI run on this
  branch is still the real test.
...
- Adding Redis/Mailpit containers and `AUTH_E2E=true` to CI was considered and deliberately
  not done — the thresholds are met without that much CI surface.

## Handoff Notes

- **`turbo.json` and the workflow are a pair.** Removing `env: ["DATABASE_URL"]` from
...
- The CI image is pinned by digest and must stay a pgvector build.
